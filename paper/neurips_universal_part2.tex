...
Of 77 public datasets (OpenML CC-18 + sklearn), 92\% have feature pairs with $|\rho| > 0.5$, and \textbf{52 (68\%)} exhibit attribution instability (flip rate $> 10\%$; model-only component ${\sim}63\%$ of total variation). The 95\% Wilson confidence interval is $[56\%, 77\%]$. For datasets with $P \geq 20$ features ($n = 43$), the rate rises to 93\% (40/43; Wilson CI: $[81\%, 98\%]$). With 20 models per dataset, the survey achieves only 32\% power at the 10\% threshold (60\% at 15\%), making 68\% a conservative lower bound; the true prevalence is likely 75--85\%.
...
\paragraph{Why this isn't trivial.}
The four-line proof may suggest the result is obvious.
It is not, for three reasons.
First, the impossibility is \emph{not} a consequence of Arrow's theorem:
Arrow aggregates many inputs into one output; our setting maps one input
(model) to one output (explanation), so Arrow's axioms do not apply.
Second, the coverage conflict diagnostic reveals a structural asymmetry
with Arrow: coverage conflict is \emph{anti-monotone} in the size of~$H$
(enlarging the explanation space weakens the impossibility), whereas
Arrow's impossibility strengthens with more alternatives.
This anti-monotonicity is why graded attributions escape the bilemma
while binary signs do not---a distinction invisible without the
formalization.
Third, the bilemma (faithful + stable alone impossible) is strictly
stronger than the trilemma for binary spaces, ruling out the natural
fallback of ``just drop decisiveness.''
The fact that this fallback works for rankings but not for circuits is a
non-obvious structural consequence of the presence or absence of neutral
elements.
