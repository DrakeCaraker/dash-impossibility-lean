...
The entire framework---322 theorems, 0 \texttt{sorry}---is formalized in
Lean~4, with the mechanistic interpretability Rashomon property
\emph{derived} from constructive types (zero axioms).
\end{abstract}
...
\item \textbf{The bilemma.}
  ...
  decomposition, feature selection, counterfactual direction.
...
\item \textbf{Lean~4 formalization.}
  The full framework is formalized across 54 files containing 322
  theorems and lemmas with 0~\texttt{sorry}.
...
The attribution impossibility for feature rankings under collinearity is
treated in full quantitative detail---including the $1/(1{-}\rho^2)$
divergence rate, Spearman instability bounds, and GBDT/Lasso/neural-net
instantiations---in a companion paper~\citep{anonymous2026attribution}.
The present paper abstracts the core mechanism into a domain-independent
framework, proves the bilemma (which does not appear in the companion
paper), and extends the impossibility to mechanistic interpretability,
causal discovery, and attention maps.
...
tested.
The full quantitative treatment---including the $1/(1{-}\rho^2)$ divergence
rate, Spearman bounds, and model-specific
instantiations---appears in the companion
paper~\citep{anonymous2026attribution}.
