...
\midrule
\multicolumn{3}{@{}l}{\emph{Query complexity (Le~Cam's method; $C\geq 1/8$ axiomatized, $C=1/12$ derived):}} \\
Testing constant & \texttt{testing\_constant} & Axiom: universal constant $C \geq 1/8$ from Tsybakov (2009) \\
Constant positivity & \texttt{testing\_constant\_pos} & Axiom: $C > 0$ (Chebyshev alternative $C=1/12$ derived in \texttt{QueryComplexityDerived.lean}) \\
\midrule
...
Spearman bound (tight) & \texttt{spearmanCorr\_bound\_groupSize\_tight} & \textbf{Theorem} in \texttt{SpearmanDef.lean} (tighter; $1-3m^2/(P^3{-}P)$) \\
...
Consensus variance bound & \texttt{consensus\_variance\_bound} & \textbf{Theorem} in \texttt{VarianceDerivation.lean} (from \texttt{IndepEnsemble}) \\
Testing lower bound & \texttt{le\_cam\_lower\_bound} & \textbf{Theorem} in \texttt{QueryComplexity.lean} (contrapositive) \\
Chebyshev query bound & \texttt{chebyshev\_query\_bound} & \textbf{Theorem} in \texttt{QueryComplexityDerived.lean} ($C=1/12$, derived) \\
Bayes-optimal tie & \texttt{bayes\_optimal\_tie} & \textbf{Theorem} in \texttt{BayesOptimalTie.lean} (symmetric posterior $\Rightarrow$ tie) \\
Pareto optimality & \texttt{dash\_pareto\_optimal} & \textbf{Theorem} in \texttt{ParetoOptimality.lean} (DASH achieves variance frontier) \\
\bottomrule
\end{tabular}
\end{table}
...
Defs.lean (14 axioms, type definitions, derived theorems)
  ...
  +-- QueryComplexity.lean (query lower bound, Le~Cam constant axiomatized $C \geq 1/8$; Chebyshev $C=1/12$ derived in QueryComplexityDerived)
  ...
  +-- UnfaithfulQuantitative.lean (quantitative unfaithfulness bounds)
  +-- BinaryQuantizer.lean (binary quantization lemmas)
  +-- BayesOptimalTie.lean (Bayes-optimal tie assignment formalized)
  +-- LossPreservation.lean (loss preservation under tie-breaking)
  +-- ParetoOptimality.lean (DASH Pareto optimality formalized)
  +-- Consistency.lean (axiom system consistency, Fin 4 model)
  +-- RandomForest.lean (contrast case, documentation only)
  +-- Setup.lean (GBDTSetup structure bundling all axioms, axiom-free API)
  ...
  +-- ApproximateEquity.lean (Rashomon from bounded proportionality K-ratio)
  +-- QueryComplexityParametric.lean (Le~Cam bound as hypothesis, no global axioms)
  +-- RobustnessLipschitz.lean (flip rate Lipschitz-continuous in rho)
  +-- StumpProportionality.lean (proportionality DERIVED for depth-1 trees)
  +-- LocalSufficiency.lean (local c suffices for all quantitative bounds)
  +-- MutualInformation.lean (impossibility generalised to I(X_j;X_k)>0)
  +-- IntersectionalFairness.lean (K-audit compounding: 1-(1/2)^K)
\end{verbatim}
...
$S \leq 1 - 3m^2/(P^3{-}P)$ (Lean: \texttt{spearmanCorr\_bound\_groupSize\_tight}).
