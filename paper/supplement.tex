...
The formalization uses 18 axioms total: 6 type/constant declarations
...
3 query-complexity axioms from Le~Cam's method. Three former axioms
...
The formalization comprises 36 Lean~4 files with 18 axioms and
188 type-checked theorems and lemmas (
...
\begin{table}[h]
\centering
\caption{Lean file cross-reference (36 files, 180 theorems, 18 axioms).}
...
GaussianFlipRate.lean & \S F1 Diagnostic & Phi, gaussianFlipRate & 2 sorry \\
...
proved in the supplement below but not
...
remains axiomatized: the full derivation via \texttt{IndepFun.variance\_sum}
would require a product measure formulation and independence axioms for
ensemble draws --- a fundamental architectural change deferred to
future work (see \texttt{docs/jmlr-variance-derivation-plan.md}).
The textbook identity $\Var(\bar{X}) = \Var(X)/M$ for i.i.d.\ variables
is the justification; the derived theorems (variance halving,
monotonicity) are genuine Lean proofs from this axiom.
...
weeks. The expansion from 49 to 180 theorems (36 files) was produced
...
no proof was accepted on the basis of AI output alone. The 75 proofs
with ${\geq}5$ tactic lines required substantive mathematical
reasoning; the deepest proof (\texttt{gaussian\_rashomon\_witnesses},
36 tactic lines) constructs explicit Rashomon witnesses via
\texttt{Real.sqrt} and \texttt{nlinarith}.

\paragraph{Proof depth distribution.}
Of 188 type-checked declarations: 75 require multi-step proofs
(${\geq}5$ tactic lines), 21 require ${\geq}10$ lines, and 3
require ${\geq}20$ lines. The five deepest proofs are
\texttt{gaussian\_rashomon\_witnesses} (36 lines, FIM ellipsoid
construction), \texttt{binary\_group\_firstmover\_is\_j\_or\_k}
(36 lines, Finset cardinality argument),
\texttt{spearmanCorr\_bound} (33 lines, midrank algebra),
\texttt{causal\_discovery\_impossibility} (18 lines, CPDAG
pigeonhole), and \texttt{sumSqRankDiff\_pos\_of\_reversal}
(16 lines, rank difference positivity). The remaining declarations
...
\paragraph{Joint consistency of all 18 axioms.}
...
The 3 query-complexity axioms (\texttt{testing\_constant},
\texttt{testing\_constant\_pos}, \texttt{le\_cam\_lower\_bound})
...
The F1 diagnostic transfers to neural networks: $r(Z, \text{flip}) = -0.871$,
...
The high F1 diagnostic correlation ($|r| = 0.871$) suggests the
...
The F1 diagnostic maintains $|r| > 0.78$ across all dimensionalities,
