...
\begin{remark}
Theorem~\ref{thm:rashomon-symmetry} shows that the models
in the main text's instantiations (the ``permuted model'' with
opposite feature utilization) are not hypothetical---they exist in
any permutation-closed model class. Combined with the observation
that any stochastic, symmetric training algorithm reaches both
sides with positive probability (by algorithmic symmetry,
$\Pr[\varphi_j > \varphi_k] = \Pr[\varphi_k > \varphi_j] = 1/2$),
this makes the impossibility inescapable for essentially any
standard ML pipeline under collinearity.
\end{remark}
...
\begin{remark}
The optimal unfaithful ranking is the population-level ranking:
$j \succ^* k \Leftrightarrow \mathbb{E}[\varphi_j] > \mathbb{E}[\varphi_k]$.
For symmetric features, $\mathbb{E}[\varphi_j] = \mathbb{E}[\varphi_k]$,
so $\succ^*$ assigns a tie---recovering the ``drop completeness''
solution. Thus \textbf{the ``drop faithfulness'' and ``drop completeness''
relaxation paths converge}: both lead to population-level attributions
with ties for symmetric features. The trilemma collapses to a single
tradeoff axis parameterized by ensemble size $M$: at $M = 1$, one has
faithfulness and completeness but no stability; as $M \to \infty$, one
gains stability at the cost of within-group completeness. Between-group
faithfulness is preserved at all $M$.
\end{remark}
