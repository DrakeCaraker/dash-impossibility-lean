...
The formalization comprises 14 Lean~4 files with 15 axioms and
44 type-checked declarations (35 theorems + 9 lemmas;
0 \texttt{sorry}). Of these, 38 require multi-line proofs
...
\begin{verbatim}
Defs.lean (15 axioms, type definitions)
  |
...
\paragraph{Attribution sum symmetry: why it remains an axiom.}
Deriving \texttt{attribution\_sum\_symmetric} from Axioms~2--4
requires two unstated assumptions: (a)~the proportionality constant
$c$ is uniform across models (Axiom~4 allows per-model $c$), and
(b)~features in a group have equal split counts when the first-mover
is in a different group (Axiom~3 only covers same-group first-movers).
Adding both as explicit axioms would increase the axiom count from~9
to~10 while eliminating only~1 axiom---a net increase. We chose to
axiomatize the conclusion directly as the most parsimonious encoding.
The justification (DGP symmetry under identical hyperparameters) is
independently verifiable empirically.
