...
42 type-checked declarations (33 theorems + 9 lemmas, of which
24 are substantive proofs; 0 \texttt{sorry}).
...
\citep{janzing2020feature}, which conditions on the causal structure
...
\subsection{The Two-Families Meta-Pattern}

The structural identity between the attribution and model selection
impossibilities suggests a general pattern:

\begin{remark}[Two-Families Conjecture]
\label{rem:two-families}
For any statistical decision problem where:
\begin{enumerate}
    \item The population distribution is \emph{symmetric} over a
      discrete choice set (which feature is dominant, which model is
      best, which causal direction holds),
    \item The observed data introduces \emph{finite-sample noise}
      sufficient to reverse the optimal choice across i.i.d.\ draws,
\end{enumerate}
the achievable (instance-accuracy, stability) set consists of exactly
two families:
\begin{itemize}
    \item \textbf{Instance-specific}: commit to the data-specific
      choice (accurate for this instance, unstable across instances).
    \item \textbf{Population-averaged}: average over instances (stable
      but losing instance-specific information).
\end{itemize}
The first instance is feature attribution (this paper). The second
is model selection (Theorem~\ref{thm:model-selection-ds}). We
conjecture the pattern extends to feature selection (Lasso
instability), causal discovery (edge orientation under
faithfulness-equivalence), and hyperparameter tuning (configuration
rankings under CV noise).
\end{remark}
