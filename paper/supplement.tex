...
axioms (\texttt{modelMeasurableSpace}, \texttt{modelMeasure}), and the
7 domain-specific property axioms below. Two former axioms
...
Attribution sum symmetry & \texttt{attribution\_sum\_symmetric} & DGP symmetry + balanced ensemble + uniform $c$ + cross-group symmetry \\
...
The formalization comprises 15 Lean~4 files with 15 axioms and
49 type-checked declarations (40 theorems + 9 lemmas;
0 \texttt{sorry}). Of these, 43 require multi-line proofs
...
\texttt{attribution\_sum\_symmetric} axiom; the variance convergence
depends on the \texttt{consensus\_variance\_bound} axiom.

\begin{verbatim}
Defs.lean (13 axioms, type definitions, derived theorems)
  ...
  +-- Corollary.lean (consensus_equity, variance_rate, variance_halves)
  +-- DesignSpace.lean (design_space_theorem, dash_produces_ties)
  +-- RandomForest.lean (contrast case, documentation only)
\end{verbatim}

\paragraph{Design Space gap.}
The Lean \texttt{design\_space\_theorem} assembles the infeasibility,
DASH equity, and variance results as a conjunction. The exhaustiveness
argument (every method falls in $\mathcal{A} \cup \mathcal{B}$) and
Pareto optimality are proved below (\S Design Space Theorem) but not
in Lean. \texttt{strongly\_faithful\_impossible} formalizes the
impossibility of consensus-value-faithful methods (a stronger result
than the supplement's Step~3, which uses ranking-faithfulness).

\paragraph{Variance axiom design choice.}
The consensus variance bound ($\Var(\bar\varphi_j) = \Var(\varphi_j)/M$)
is axiomatized directly rather than derived from Mathlib's measure-theoretic
infrastructure (\texttt{IndepFun.variance\_sum}). The alternative would
require 4 additional axioms (\texttt{MeasurableSpace Model},
\texttt{MeasureTheory.Measure Model}, \texttt{Measurable (attribution fs j)},
and an independence axiom), increasing the total axiom count from 15 to 19
while adding significant type-theoretic boilerplate. We chose the single
intuitive axiom because: (i) $\Var(\bar{X}) = \Var(X)/M$ for i.i.d.\
random variables is a textbook identity requiring no justification beyond
the independence assumption; (ii) the 4 measure-theoretic axioms are
harder to audit (they assert abstract mathematical structure on our
\texttt{Model} type, which has no natural measurable-space interpretation);
and (iii) the derived theorems (variance halving, nonnegativity) are
genuine Lean proofs, not tautologies from the axiom.

\paragraph{Spearman bound: axiomatized vs.\ derived.}
The Lean formalization contains two Spearman bounds:
(i)~\texttt{spearmanCorr\_bound}, fully derived from the definition of
Spearman correlation, giving the weaker bound
$\rho_S \leq 1 - 3/(P^3 - P)$; and
(ii)~\texttt{spearman\_classical\_bound}, axiomatized, giving the
tighter $\rho_S \leq 1 - m^3/P^3$ from the classical combinatorial
argument. The paper's stability results use the tighter axiomatized
bound. The derived bound is available but superseded. We chose to
axiomatize the tighter constant rather than derive it because the
full combinatorial proof (bounding the expected $\sum d_i^2$ under
random tie-breaking) requires counting arguments not yet available
in our Lean framework.

\paragraph{Attribution sum symmetry: derivation infrastructure.}
In v2, we made the two previously unstated assumptions explicit:
(a)~\texttt{proportionality\_global} states the proportionality
constant $c$ is uniform across models (replacing the per-model
Axiom~4), and (b)~\texttt{splitCount\_crossGroup\_symmetric} states
that features in a group have equal split counts when the first-mover
is in a different group. Both are DGP-structural axioms.
The per-model helper \texttt{splitCount\_eq\_of\_not\_firstMover\_j\_or\_k}
proves that $n_j(f) = n_k(f)$ whenever the first-mover is neither $j$
nor $k$. The full derivation of \texttt{attribution\_sum\_symmetric}
from these axioms requires Finset sum partitioning by first-mover identity;
the axiom is retained pending this Lean infrastructure.
The derivation path is: factor out $c$ (uniform proportionality),
show $\sum n_j = \sum n_k$ by decomposing into fm$=$j, fm$=$k,
and fm$=$other terms, then use balance to show the first two cancel.

\paragraph{Variance sub-system.}
The three variance axioms (\texttt{attribution\_variance},
\texttt{attribution\_variance\_nonneg}, \texttt{consensus\_variance\_bound})
produce convenience results (variance nonnegativity, halving) rather than
deep mathematical consequences. Their primary role is to support the
Design Space Theorem's stability convergence claim. A deeper formalization
would derive the variance bound from Mathlib's
\texttt{IndepFun.variance\_sum}, but this requires connecting our abstract
\texttt{Model} type to a measure space (see above).
...
For practitioners, this means the F1 diagnostic ($Z_{jk}$ threshold)
...
The code supplement also includes the F5$\to$F1$\to$DASH diagnostic
...
ceiling), and F1 diagnostic correlation $|r| > 0.89$ in all three.
The instability is a property of sequential boosting under
collinearity, not of any specific implementation.

\paragraph{F5 caveat.}
The F5 split-frequency diagnostic requires access to per-tree split
...
threshold. For CatBoost, the F1 multi-model diagnostic ($r = -0.892$)
...
Note: the F5 precision values in Table~\ref{tab:cross-impl} use overall
feature importance values (\texttt{feature\_importances\_}), not the
per-tree split indicators described in \S F5. The per-tree F5 diagnostic
...
per-tree F5 is recommended for production use.
The reference implementation of the full \DASH{} pipeline is available
at \url{https://github.com/DrakeCaraker/dash-shap}.
...
internal randomness), we train 50~models on 50 different 80\% bootstrap
samples of the training data, with \texttt{subsample=1.0} and a fixed
...
\begin{itemize}
  \item \textbf{If subsample $< 1.0$ (stochastic, recommended):}
    Run F5 (single-model screen) $\to$ F1 (5-model validation) $\to$
  ...
  \item \textbf{If subsample $= 1.0$ (deterministic):} A single
    ...
    produces a new model that may rank features differently. Run F1
...
theoretical maximum flip rate (0.500). The F1 diagnostic achieves
$|r| > 0.89$ for both, confirming the diagnostic is method-agnostic.
...
$S \leq 1 - m^3/P^3$ even when all between-group $S_{jk}$ are high.
\end{definition}
...
\paragraph{Step 1 (F5).} A single model flags 4 of the 10 tracked pairs
...
(max flip rate 50\%, confirming the F1 analysis). The impossibility is not a
...
coherence, and the F1 diagnostic $Z_{jk}$ plays the role of the
...
``known and foreseeable circumstances'' affecting accuracy. Our
theorem proves that this includes: ``SHAP-based proxy audits are
