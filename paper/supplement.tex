...
\setcounter{theorem}{5}    % main ends at Corollary 5 (DASH equity)
\setcounter{axiom}{1}      % main has Axiom 1
\setcounter{figure}{2}     % main has Figures 1--2
\setcounter{table}{4}      % main has Tables 1--4 (cc-comparison, ranking-lottery, clinical, tinystories)
...
The formalization uses 6 axioms total: 2 type declarations
(\texttt{Model}, \texttt{firstMover}), 3 domain-specific property axioms
(\texttt{firstMover\_surjective}, \texttt{crossGroupBaselineCore},
\texttt{proportionalityConstant}), and 1 measure-theoretic axiom
(\texttt{modelMeasure}).
Ten former axioms---including the split-count function, 4 split-count
properties, attribution function, proportionality, boosting rounds,
measurable space, and query-complexity constant---are now definitions or
derived theorems (see axiom reduction history in main text).
\end{sloppypar}
...
Split count (non-first-mover) & \texttt{splitCount\_\allowbreak nonFirstMover} & \textbf{Theorem}: derived from \texttt{splitCount} definition \\
...
\midrule
\multicolumn{3}{@{}l}{\emph{Additional derived theorems:}} \\
Spearman bounds & \texttt{spearman\_instability\_bound} & \textbf{Theorem} in \texttt{SpearmanDef.lean} (from midranks) \\
Attribution sum symmetry & \texttt{attribution\_sum\_symmetric} & \textbf{Theorem} in \texttt{SymmetryDerive.lean} \\
Variance bound & \texttt{consensus\_variance\_bound} & \textbf{Theorem} in \texttt{VarianceDerivation.lean} \\
Chebyshev query bound & \texttt{chebyshev\_query\_bound} & \textbf{Theorem} in \texttt{QueryComplexityDerived.lean} ($C=1/12$) \\
Bayes-optimal tie & \texttt{bayes\_optimal\_tie} & \textbf{Theorem} in \texttt{BayesOptimalTie.lean} \\
Pareto optimality & \texttt{dash\_pareto\_optimal} & \textbf{Theorem} in \texttt{ParetoOptimality.lean} \\
\bottomrule
\end{tabular}
\end{table}
...
The formalization comprises 58 Lean~4 files with 6 axioms and
357 type-checked theorems and lemmas (
0 \texttt{sorry}). Of these, 139 require multi-line proofs
...
\midrule
Defs.lean & \S1 (Axiom System) & 6 axioms + derived theorems & No \\
...
MechInterp.lean & \S Binary & mech\_interp\_bilemma & No \\
BeyondBinary.lean & \S Binary & beyond\_binary\_extensions & No \\
...
weeks. The expansion from 49 to 357 theorems (58 files) was produced
...
no proof was accepted on the basis of AI output alone. The 110 proofs
...
Of 357 type-checked declarations: 139 require multi-step proofs
...
\paragraph{Joint consistency of all 6 axioms.}
The 6 axioms are jointly consistent: \texttt{Consistency.lean}
constructs an explicit model ($P{=}4$, $L{=}2$, $m{=}2$, $\rho{=}0.5$,
$T{=}100$, Fin~4 models) satisfying all 6 simultaneously.
...
(the Rashomon property, \S2 of the main text). By Theorem~1,
...
For any model class satisfying the Rashomon property (\S2 of the main text)
...
By the Rashomon property (\S2 of the main text), for any within-group pair
...
predictors'': if the dataset has correlated features (68\% of
...
We extend the replication study to 5 public datasets, counting how many distinct top-$K$ SHAP rankings appear across 50 seeds (Table~\ref{tab:ranking-lottery-supp}).

\begin{table}[t]
\centering
\caption{The ranking lottery: distinct top-3 and top-5 SHAP rankings across 50 seeds (\texttt{subsample=0.8}, fixed test set). Deterministic control (\texttt{subsample=1.0}): 1 ranking for all datasets.}
\label{tab:ranking-lottery-supp}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
Dataset & Samples & Distinct top-3 & Top-3 agree & Distinct top-5 & Top-5 agree \\
\midrule
Breast Cancer & 569 & \textbf{24} & \textbf{4.2\%} & \textbf{46} & \textbf{0.4\%} \\
Diabetes & 768 & 2 & 88.5\% & 4 & 68.2\% \\
Wine Quality & 1,599 & 1 & 100\% & 2 & 55.6\% \\
Heart Disease & 270 & 1 & 100\% & 1 & 100\% \\
California Housing & 20,640 & 1 & 100\% & 1 & 100\% \\
\bottomrule
\end{tabular}
\end{table}

Seven distinct features compete for the top-3 positions on Breast Cancer: worst concave points, mean concave points, worst area, worst concavity, worst texture, worst perimeter, and area error. The \#1 position alternates: worst concave points (50\%), mean concave points (32\%), worst area (18\%). At 100 seeds: 35 distinct top-3 (up from 24) and 84 distinct top-5 (up from 46)---the count keeps growing, confirming 24 is a lower bound on the number of possible rankings.

\paragraph{Subsample sensitivity.}
The instability persists even at reduced stochasticity. On Breast Cancer (50 seeds):

\begin{center}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\texttt{subsample} & Distinct top-3 & Agreement \\
\midrule
0.80 & 24 & 4.2\% \\
0.85 & 23 & 5.2\% \\
0.90 & 22 & 6.9\% \\
...
Only fully deterministic training eliminates the lottery. At \texttt{subsample=0.95}, 17 distinct rankings remain. ``Just use less stochasticity'' is not a resolution.

\paragraph{Cross-implementation.}
On Breast Cancer: XGBoost 24 distinct top-3 (4.2\% agreement), LightGBM 29 (3.6\%, \texttt{subsample\_freq=1}), Random Forest 40 (2.5\%, permutation importance). The instability is implementation-independent.

\paragraph{Gene count robustness.}
On AP\_Colon\_Kidney, the ranking lottery result is robust to the number of pre-selected genes: 8 distinct top-3 at 20 genes, 5 at 50, 7 at 100. The instability is not an artifact of the filtering threshold.

\subsection{Gene Expression: Pathway-Level Consequence}

On a 10{,}935-gene colon-vs-kidney expression dataset (AP\_Colon\_Kidney, 546 samples, top 50 genes by variance, OpenML), the \#1 biomarker gene alternates between TSPAN8 (203824\_at, invasion/metastasis pathway, 80\% of seeds) and CEACAM5/CEA (201884\_at, immune evasion pathway, 20\% of seeds). Both are cell adhesion molecules enriched in colon tissue ($\rho = 0.858$), but with distinct cancer biology roles: TSPAN8 in invasion/metastasis signaling via tetraspanin-enriched microdomains, and CEACAM5 (the classical clinical colon cancer marker CEA) in immune evasion and differentiation via GPI-anchored adhesion.

The broad pathway-level conclusion (colon identity genes drive the distinction) is preserved, but the specific biomarker target differs. A biomarker development pipeline reporting TSPAN8 as the top discriminator would target a different gene than one reporting CEA. DASH consensus ranks both in the top-2 (TSPAN8: importance 1.913; CEACAM5: 1.553), correctly reporting that both contribute without privileging either.

Probe-to-gene mappings verified via the Affymetrix HG-U133A annotation database.

\paragraph{Positive control.}
On AP\_Breast\_Lung (breast vs lung, 470 samples, 10{,}935 genes), the top-3 genes are stable (2 distinct rankings, 92\% agreement) despite 171 highly correlated gene pairs ($|\rho| > 0.8$), because the top gene's importance (2.90) dominates the second (1.40) with a clear gap.
...
``known and foreseeable circumstances'' which may lead to risks to health, safety or fundamental rights. We argue
that our theorem identifies such a circumstance: in our reading, SHAP-based proxy audits are
...
The framework's tightness classification applies to the Chouldechova--Kleinberg fairness impossibility (calibration $P_1$ + equal FPR $P_2$ + equal FNR $P_3$ cannot coexist when base rates differ). Under performance constraints (TPR $\geq \tau$ for both groups):

\begin{center}
\small
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Constraint} & \textbf{Tightness type} & \textbf{Achievable pairs} \\
\midrule
$\tau = 0$ (unconstrained) & Full & All three pairs (degenerate classifiers) \\
$\tau \geq 0.3$ & PPV-blocked & Only $P_2{+}P_3$ (equalized odds) \\
\bottomrule
\end{tabular}
\end{center}

\noindent At any practical performance level, calibration (PPV parity) is individually incompatible with both equal FPR ($P_1{+}P_2$ gap $> 0.10$) and equal FNR ($P_1{+}P_3$ gap $> 0.13$). The only achievable pairwise fairness compromise is equalized odds ($P_2{+}P_3$ gap $< 0.006$). Validated on Adult Income (sex, base rate gap 0.20) and synthetic data (base rate gap 0.25--0.40) with bootstrap model retraining (20 iterations, 100\% stable). This finding changes the practical recommendation from ``choose a fairness criterion'' to ``equalized odds is the only achievable criterion at practical performance; calibration is structurally incompatible.''

\paragraph{Reynolds best-approximation optimality.}
The orbit average $Rv = \frac{1}{|G|}\sum_{g \in G} g \cdot v$ is the \emph{closest $G$-invariant vector} to $v$ in $L^2$ norm. For any $G$-invariant $w$ (i.e., any stable approximation): $\|v - Rv\| \leq \|v - w\|$. The proof is a direct consequence of the Pythagorean decomposition: $v = Rv + (v - Rv)$ where $Rv \in V^G$ and $(v - Rv) \perp V^G$. Lean: \texttt{reynolds\_best\_approximation} (idempotent self-adjoint projection). This establishes that \DASH{} and $V^G$ minimize information loss among \emph{all} stable methods --- not just unbiased or linear ones.
