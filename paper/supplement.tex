...
Attribution sum symmetry & \texttt{attribution\_sum\_symmetric} & \textbf{Theorem} in \texttt{SymmetryDerive.lean} (from axioms above) \\
...
The $Z$-test (equivalently, the F1 diagnostic with optimal $M$) detects a
...
Aggregation type & Many inputs $\to$ one output & One input $\to$ one output (for each model) \\
