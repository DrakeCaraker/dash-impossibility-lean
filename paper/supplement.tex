...
The F1 diagnostic achieves $|r| > 0.8$ on 7/10 datasets with unstable
pairs (all except Digits, Adult, and Iris). The weaker correlations on
Digits ($r = -0.42$) and Adult ($r = -0.29$) reflect a floor effect:
these high-dimensional datasets have many features with near-zero
importance, producing many pairs where both $Z$ and flip rate are near
zero (contributing noise to the correlation). Restricting to pairs with
$Z < 5$ (the diagnostically interesting range) would improve these
correlations, as demonstrated for Breast Cancer in the restricted-range
analysis above.

F5 precision is 94--100\% on small/clean datasets (Breast Cancer, Wine,
Heart Disease, Communities) where split counts are reliable, but drops to
23--58\% on high-dimensional datasets (Adult, Digits, Ames) where split
counts are sparse and noisy. This confirms F5 as a conservative
...
We show that any method $A$ must fall in one of the two families.
