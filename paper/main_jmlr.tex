...
The failure is explained by the \textbf{Explanation Capacity Theorem} (Lean-verified in \texttt{SAECapacity.lean}): for an $n$-feature SAE on $d$-dimensional data, the reproducible subspace has dimension $\leq d$; $C \leq d/n = 12.5\%$. The correct resolution is projection onto data principal components (seed-invariant basis). Empirically: projecting feature importance onto PCs of the model's layer-6 activations achieves $\rho = 0.960$ (vs.\ raw $-0.002$, matched $0.500$, and $G$-invariant for heads $0.873$). SAE importance IS reproducible when you ask ``which data directions carry importance?'' rather than ``which features are important?'' The PC projection is the Reynolds operator for the dictionary's continuous symmetry group.
