...
The failure is explained by the \textbf{Explanation Capacity Theorem} (Lean-verified in \texttt{SAECapacity.lean}): for an $n$-feature SAE on $d$-dimensional data, the reproducible subspace has dimension $\leq d$. The explanation capacity $C \leq d/n = 768/6144 = 12.5\%$; the loss rate $\eta \geq 87.5\%$. The 5{,}376-dimensional null space guarantees non-reproducibility by rank-nullity, not by training methodology. The correct resolution is projection onto data principal components (seed-invariant), analogous to per-layer averaging for heads.
