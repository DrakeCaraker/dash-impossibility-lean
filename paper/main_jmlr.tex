...
\begin{abstract}
No feature ranking can be simultaneously faithful, stable, and complete under collinearity. We prove this impossibility, characterize the complete design space (exactly two achievable families), and give the optimal resolution among unbiased aggregations.
...
\DASH{} (\textbf{D}iversified \textbf{A}ggregation of \textbf{SH}AP) ensemble averaging is Pareto-optimal among unbiased aggregations on the stable branch, achieving the Cram\'er--Rao variance bound with a tight ensemble size formula.
In a survey of 77 public datasets, 68\% exhibit attribution instability (95\% CI: [56\%, 77\%]; rising to 93\% for $P \geq 20$, 95\% CI: [81\%, 98\%]).
...
\begin{table}[h]
\centering
\caption{F1 and F5 diagnostic generality across 11 datasets (see also Figure~\ref{fig:comprehensive-f1}).}
...
\DASH{} requires training $M$ models with different seeds, computing SHAP values for each, and averaging the absolute SHAP values across models. A reference implementation with the full F5$\to$F1$\to$\DASH{} diagnostic workflow is available at \url{https://github.com/DrakeCaraker/dash-shap}.
...
The formalization comprises 46 Lean~4 files with 16 axioms and 260 type-checked theorems and lemmas (0 \texttt{sorry}). Of these, 67 require multi-line proofs ($\geq$5 tactic lines); the remainder are definitions, wrappers, and single-step applications. The core impossibility (\texttt{attribution\_impossibility}) depends on zero axioms---only the Rashomon property as hypothesis. The DASH resolution (\texttt{consensus\_equity}) depends on the \texttt{attribution\_sum\_symmetric} theorem (derived from axioms); the variance convergence depends on the \texttt{consensus\_variance\_bound} theorem (also derived).

\paragraph{Scope of verification.}
The Lean formalization covers the impossibility theorems, design space dichotomy (Steps 1, 2, 4 of Theorem~\ref{thm:design-space-main}), all three SBD instances, and the quantitative bounds. The exhaustiveness argument (Step~3, Case~2: Bayes-optimality of ties for symmetric features) requires measure-theoretic decision theory not currently available in Mathlib and is proved only on paper. The consensus variance bound ($\Var(\bar\varphi_j) = \Var(\varphi_j)/M$) is axiomatized rather than derived from independence; the full derivation via \texttt{IndepFun.variance\_sum} would require product measure formulations deferred to future work.
...
The Lean formalization derives the Spearman bound \texttt{spearman\_instability\_bound} from the definition of Spearman correlation via midrank algebra, giving $\rho_S \leq 1 - 3(m{-}1)^2/(P^3{-}P)$. The paper uses the tighter classical bound $\rho_S \leq 1 - m^3/P^3$; closing the gap between the two requires a combinatorial counting argument (bounding the effect of transposition on midranks in the expected $\sum d_i^2$ under random tie-breaking) that is not a domain-specific assumption but requires combinatorial lemmas not yet available in the Lean framework.
...
\midrule
\multicolumn{4}{@{}l}{\emph{Axiom strengthening and extensions (not individually referenced above):}} \\
Qualitative.lean & Stratification & impossibility\_qualitative & No \\
ProportionalityLocal.lean & Stratification & gbdt\_impossibility\_local & No \\
LocalSufficiency.lean & Stratification & local\_proportionality\_suffices & No \\
StumpProportionality.lean & \S\ref{sec:gbdt-bounds} & stump\_proportionality\_unique & No \\
IntersectionalFairness.lean & \S\ref{sec:fairness} & intersectional\_audit\_impossibility & No \\
MutualInformation.lean & \S\ref{sec:discussion} & mi\_is\_exact\_boundary & No \\
Setup.lean & Bundled & attribution\_impossibility\_bundled & No \\
ApproximateEquity.lean & Stratification & rashomon\_from\_bounded\_proportionality & No \\
QueryComplexityParametric.lean & \S\ref{sec:query} & query\_complexity\_parametric & No \\
\bottomrule
\end{tabular}
\end{table}
...
In a survey of 77 public datasets, 92\% have feature pairs with $|\rho| > 0.5$ and 68\% exhibit attribution instability (93\%, CI: [81\%, 98\%], for $P \geq 20$)---the impossibility is a practical reality. Our theorem proves this is a ``known and foreseeable circumstance'' under EU AI Act Art.~13(3)(b)(ii) \citep{euaiact2024}.
