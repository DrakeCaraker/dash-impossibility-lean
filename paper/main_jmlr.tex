...
\begin{abstract}
Feature rankings from SHAP change when you retrain the model---in 68\% of 77 public datasets (under the standard workflow; model variation accounts for ${\sim}63\%$ of the total), the ``most important'' feature flips with the random seed. We prove this is not an engineering problem but a mathematical impossibility: no feature ranking can be simultaneously faithful (reflecting the model), stable (consistent across retrains), and complete (ordering all features) when features are collinear with similar true importance. The impossibility is quantitative, with architecture-discriminating bounds: the attribution ratio diverges as $1/(1{-}\rho^2)$ for gradient boosting and is infinite for Lasso, while random forests converge. The achievable design space consists of exactly two families; \DASH{} ensemble averaging is Pareto-optimal among unbiased aggregations on the stable branch, achieving equitable attributions with Cauchy--Schwarz-optimal variance. The impossibility, design space, and resolution are mechanically verified in Lean~4 (357 theorems, 6 axioms, 0~\texttt{sorry}; 139 with multi-step proofs of ${\geq}5$ tactic lines)---the first formally verified impossibility result in explainable AI.
\end{abstract}
...
Train a gradient-boosted model on your data. Compute SHAP values. Retrain with a different random seed. The `most important feature' changes---in 68\% of 77 public datasets (under the standard workflow; on Breast Cancer, \S~\ref{sec:experiments} isolates the model-only component at approximately 63\% of the total variation). Before this work, practitioners assumed this was an engineering problem. We prove it is a mathematical impossibility---and we provide the complete theory: an impossibility theorem, quantitative architecture-discriminating bounds, a constructive resolution with provable optimality, and machine verification in Lean~4.
...
The formalization comprises 58 Lean~4 files with 6 axioms and 357 type-checked theorems and lemmas (0~\texttt{sorry}). Of these, 139 require multi-line proofs ($\geq$5 tactic lines); the remainder are definitions, wrappers, and single-step applications.
...
\textbf{AI-assistance disclosure.} The Lean 4 formalization was developed with substantial assistance from Claude Code (Anthropic, Claude Opus 4.6). All 6 axioms and all theorem statements were human-conceived. Approximately 80\% of proof tactics were initially AI-drafted and then human-verified. Every proof was accepted only after passing \texttt{lake build} (the Lean 4 type-checker) with zero \texttt{sorry}. The paper text was written with AI drafting and editing assistance; all mathematical claims, experimental designs, and interpretations are the responsibility of the human authors. We view AI-assisted formal verification as a methodology contribution: the type-checker guarantees correctness given the axioms, while human judgment ensures the axioms capture the intended mathematics.

\subsection{Proof Depth Distribution}

Of 357 type-checked declarations: 139 require multi-step proofs (${\geq}5$ tactic lines), 21 require ${\geq}10$ lines, and 5 require ${\geq}20$ lines. The five deepest proofs are listed below.
...
Preliminary evidence suggests the impossibility extends to mechanistic interpretability: 10 independently grokked transformers on modular addition (all 100\% accuracy) agree on only 36\% of the top-3 circuit components by activation patching importance (min pairwise Spearman $= -0.18$).
