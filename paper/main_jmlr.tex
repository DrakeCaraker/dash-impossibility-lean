...
The failure is explained by the \textbf{Explanation Capacity Theorem} (Lean-verified in \texttt{SAECapacity.lean}): for an $n$-feature SAE on $d$-dimensional data, the reproducible subspace has dimension $\leq d$; $C \leq d/n = 12.5\%$. The resolution: project onto PCs of the model's layer-6 activations (SAE-seed-invariant). Result: $\rho = 0.960$ for decoder coverage geometry (vs.\ raw $-0.002$, matched $0.500$). The dictionary's \emph{aggregate coverage} of data directions is near-perfectly reproducible; individual feature assignments are not. Note: with unit-norm decoders (TopK normalization), this measures directional density, not activation-weighted importance; the latter remains an open test. The $G$-invariant comparison for heads ($\rho = 0.873$) involves a harder task (10 different models vs.\ 10 SAEs on one frozen model) and is not directly comparable.
