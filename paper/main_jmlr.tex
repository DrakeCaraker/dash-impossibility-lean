...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (350 type-checked theorems from 10 axioms, 0~\texttt{sorry}; 115 with multi-step proofs of ${\geq}5$ tactic lines), publicly available at \url{https://github.com/DrakeCaraker/dash-impossibility-lean} (archived at \url{https://doi.org/10.5281/zenodo.19468379}).\linebreak The axiom system is consistent: we construct an explicit model satisfying all 14 domain-specific axioms simultaneously, both in Lean (\texttt{Consistency.lean}) and~numerically.
\end{enumerate}
...
The formalization uses 10 axioms total: 5 type/constant declarations, 2 measure-theoretic infrastructure axioms, and 3 domain-specific property axioms. The split-count structure and query-complexity constant are now derived (see \S~\ref{sec:lean}).
...
The formalization comprises 58 Lean~4 files with 10 axioms and 350 type-checked theorems and lemmas (0~\texttt{sorry}). Of these, 115 require multi-line proofs ($\geq$5 tactic lines); the remainder are definitions, wrappers, and single-step applications.
...
The initial formalization (49 theorems, 15 files) was developed by the first author with iterative Claude Code assistance over several weeks. The expansion from 49 to 350 theorems (58 files) was produced in a single development session by dispatching targeted Lean-writing agents with detailed mathematical specifications. All proofs were machine-verified by \texttt{lake build} (the Lean~4 type-checker); no proof was accepted on the basis of AI output alone.
...
\begin{table}[t]
\centering
\caption{Lean file cross-reference (58 files, 350 theorems, 10 axioms).}
...
The formalization uses 10 axioms total (8 domain-specific + 2 measure-theoretic), but not all results require all axioms. Six former axioms (split-count function, 4 split-count properties, and the query-complexity constant) are now definitions or derived theorems. The Lean type-checker's \texttt{\#print axioms} command verifies exact dependencies:
\begin{itemize}
