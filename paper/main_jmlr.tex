...
Let $A$ be a stochastic training algorithm with continuous dependence on its random seed. For any DGP with $\rho > 0$ and $n < \infty$ training samples, $\Pr[\varphi_j(f) \neq \varphi_k(f)] = 1$ for features $j, k$ in the same collinear group.
\end{assumption}

\begin{remark}
Non-degeneracy is empirically universal (verified across all experiments in this paper) but formally unproved; a rigorous proof requires a transversality argument. The core impossibility (Theorem~\ref{thm:impossibility}) does not depend on this assumption. Theorem~\ref{thm:rashomon-inevitability} uses it to show Rashomon is \emph{inevitable}, not merely possible.
\end{remark}

\begin{theorem}[Rashomon inevitability]
\label{thm:rashomon-inevitability}
Let $A$ be a stochastic, symmetric training algorithm for a permutation-closed model class $\mathcal{F}$. Under non-degeneracy (Assumption~\ref{thm:non-degeneracy}), for any DGP with $\rho > 0$, the Rashomon property holds: for any $j, k$ in the same collinear group, there exist models $f, f'$ in the support of $A$ with $\varphi_j(f) > \varphi_k(f)$ and $\varphi_k(f') > \varphi_j(f')$.
\end{theorem}
