...
\author{\Name{Drake Caraker} \Email{drakecaraker@gmail.com}\\
  ...
  \AND
  \Name{Bryan Arnold} \Email{}\\
  \addr Independent Researcher
  \AND
  \Name{David Rhoads} \Email{}\\
  \addr Independent Researcher
}
...
\begin{abstract}
No feature ranking can be simultaneously faithful, stable, and complete under collinearity. We prove this impossibility, characterize the complete design space (exactly two achievable families), and give the provably optimal resolution.
...
In a survey of 77 public datasets, 68\% exhibit attribution instability (95\% CI: [56\%, 77\%]; rising to 93\% for $P \geq 20$).
...
\begin{keywords}
Feature attribution impossibility, Shapley value stability, Rashomon effect, ensemble explanation methods, formal verification (Lean 4)
\end{keywords}
...
Train a gradient-boosted model on your data. Compute SHAP values. Retrain with a different random seed. The `most important feature' changes. In a survey of 77 public datasets, 68\% exhibit this instability. Before this work, practitioners assumed this was an engineering problem. We prove it is a mathematical impossibility---and we provide the complete theory: an impossibility theorem, quantitative architecture-discriminating bounds, a constructive resolution with provable optimality, and machine verification in Lean~4.

For $m{=}2$, ranking collinear pairs is literally a coin flip.

The classical multicollinearity concern is about estimation variance. Our result is qualitatively different: it concerns \emph{ranking impossibility}. Even with perfect estimates, the ranking of symmetric features is a coin flip.
...
The proof is mechanically verified in Lean~4 using Mathlib: 260 type-checked theorems and lemmas across 46 files (75 with multi-step proofs of ${\geq}5$ tactic lines; the remainder are definitions, wrappers, and single-step applications; from 14 domain-specific + 2 query-complexity axioms; 0 \texttt{sorry}).
...
\begin{table}[h]
\centering
\caption{Complete axiom inventory (17 total).}
\label{tab:axioms}
\small
...
Subsampled SHAP    & 16.3\% [13.8, 18.5] & 16.1\% [14.2, 17.9] & 18.8\% [---]         & 40/40 \\
\textbf{\DASH{} ($M{=}25$)} & \textbf{3.8\%} [2.8, 4.9] & \textbf{3.1\%} [2.4, 3.9] & \textbf{3.7\%} [2.7, 4.7] & 40/40 \\
CI SHAP            & 0.2\% [0.1, 0.4]    & 0.8\% [0.6, 1.1]    & 3.1\% [---]          & $\sim$22/40 \\
\bottomrule
\end{tabular}
\end{table}

Bootstrap and subsampled SHAP address SHAP \emph{estimation} noise (a single-model phenomenon) but not the Rashomon effect (a multi-model phenomenon). Their flip rates match the single-model baseline. Only \DASH{} trains multiple models, achieving a 4--5$\times$ reduction.

CI SHAP achieves lower flip rates than \DASH{} by refusing to rank pairs whose confidence intervals overlap---resolving only $\sim$55\% of within-group pairs. This is exactly the impossibility theorem in action: CI SHAP trades completeness for stability. At $\rho = 0.9$, its flip rate converges toward \DASH{}'s ($3.1\%$ vs.\ $3.7\%$) as overlapping CIs force more pairs into ``tied'' status.
...
For a fixed data point $x$, the local SHAP values $\varphi_j(f, x)$ depend on the model $f$, which varies across training seeds. The global attribution $\varphi_j(f) = \E_x[|\varphi_j(f, x)|]$ averages over data points, which can only reduce variance (by Jensen's inequality applied to the convex function $|\cdot|$):
\[
    \Var_f\!\big(\varphi_j(f)\big) = \Var_f\!\big(\E_x[|\varphi_j(f,x)|]\big) \leq \E_x\!\big[\Var_f(|\varphi_j(f,x)|)\big].
\]
The right side is the average local instability; the left side is the global instability. Since the flip rate is a monotone function of the variance-to-gap ratio $\sigma/\Delta$, and averaging reduces $\sigma$ without changing $\Delta$, the global flip rate is a lower bound on the average local flip rate.

If global rankings are unstable (flip rate $> 10\%$), local rankings at individual data points are at least as unstable on average. Practitioners who report per-instance SHAP explanations face the same or worse instability. The F1 diagnostic (computed on global attributions) is therefore a conservative screen for local instability.
...
``The correlated group \{X, Y, Z\} contributes a total \DASH{} attribution of [value] to the prediction ($[percent]\%$ of total). Within this group, individual feature rankings are unstable. The group's total importance is stable; individual importance within the group should be interpreted as interchangeable.''
...
To validate on the exact collinearity pattern common in US credit models, we generate 10{,}000 samples with 5 features driven by a latent ``creditworthiness'' factor: income ($\rho = 0.99$ with DTI), DTI, credit score ($\rho = 0.98$ with income), loan amount (independent), and employment years ($\rho = 0.94$ with income). With 25 XGBoost models: income$\leftrightarrow$DTI flips 8\% ($|\rho| = 0.99$); credit score$\leftrightarrow$employment years flips 12\% ($|\rho| = 0.92$). Pairs where one feature clearly dominates (income vs.\ credit score) show 0\% flips despite $|\rho| = 0.98$---confirming the impossibility requires BOTH high correlation AND similar importance. For SR~11-7 \cite{occ2011sr117} compliance: report income and DTI as ``interchangeable key risk drivers'' rather than forcing a ranking that would flip under retraining.

\paragraph{Lending Club (positive control).}
Zero unstable pairs despite correlated features ($Z > 36$ for all pairs), confirming the impossibility requires \emph{both} collinearity AND similar true importance. Three correlated groups detected: person\_age$\leftrightarrow$cb\_person\_cred\_hist\_length ($|\rho| = 0.86$), loan\_amnt$\leftrightarrow$loan\_percent\_income ($|\rho| = 0.58$), loan\_int\_rate$\leftrightarrow$cb\_person\_default\_on\_file ($|\rho| = 0.50$). All have $Z > 36$, confirming distinguishable importance. Not every correlated dataset exhibits instability. The diagnostic distinguishes datasets where instability is a genuine concern from those where it is not.

\subsection{LLM Attention Attribution Instability}

We provide preliminary evidence that attribution instability extends to transformer attention: fine-tuning DistilBERT on SST-2 with different seeds produces 14.5\% unstable adjacent-token pairs. However, this is a proxy experiment---attention weights are not SHAP values---and full LLM generalization remains an open problem.
...
Of 77 public datasets surveyed (OpenML CC-18 benchmark suite + sklearn), 71 (92\%) have feature pairs with $|\rho| > 0.5$, and 52 (68\%, 95\% Wilson CI: [56\%, 77\%]) exhibit attribution instability (flip rate $> 10\%$). For datasets with $P \geq 20$ features, the instability rate rises to 93\%. With 20 models per dataset, the measured flip rate exceeds the 10\% threshold with approximately 60\% power, so the true prevalence may be higher still.
...
\subsection{Additional Experimental Validation}
\label{sec:additional-experiments}

We conducted 12 additional experiments to test the robustness and generalizability of the impossibility. Results are available in the repository as JSON with bootstrap confidence intervals.

\paragraph{Mechanism validation.}
With deterministic training (\texttt{subsample=1.0}), the flip rate drops to 0.0\% (95\% CI: [0.0\%, 0.1\%]), compared to 22.8\% [21.5\%, 24.2\%] with stochastic training (\texttt{subsample=0.8}). This confirms the Rashomon mechanism: stochastic training creates the Rashomon set.

\paragraph{Flip rate formula validation.}
Monte Carlo simulation (1,000 trials per $\rho$, stumps) validates $\Phi(-\text{SNR})$ to within 2.6\% across $\rho \in \{0.1, \ldots, 0.99\}$.

\paragraph{Conditional SHAP.}
Using SHAP's interventional mode, conditional SHAP gives 50.3\% flip rate at $\Delta\beta = 0$ (equal causal effects)---confirming the impossibility holds for conditional SHAP under symmetry. Conditional SHAP resolves the ranking only when causal effects differ ($\Delta\beta > 0.1$). Decorrelation (residualization) always gives 0\% flip rate but changes the feature semantics---it does not escape the impossibility, it changes the problem.

\paragraph{Hyperparameter universality.}
Across 81 XGBoost configurations (3 learning rates $\times$ 3 depths $\times$ 3 tree counts $\times$ 3 correlations), the minimum flip rate is 38.7\% [37.1\%, 40.2\%] at $\rho = 0.5$. No hyperparameter configuration eliminates instability for $\rho > 0$.

\paragraph{Compounding factors.}
Class imbalance amplifies instability (max flip rate increases monotonically with imbalance ratio). Missing data compounds the effect across all mechanisms (MCAR, MAR, MNAR) and rates (5\%--20\%). Instability persists and grows over 50 retrain cycles with simulated data drift.

\paragraph{Generality beyond tabular GBDT.}
In bag-of-words text classification (20newsgroups, TF-IDF + XGBoost), 60\% of documents have unstable top-1 token attribution and 91\% have unstable top-3. Temporal feature engineering (AR(1) processes with rolling windows) creates correlated groups with 27\% of within-series pairs exhibiting instability.

\paragraph{Regulatory impact.}
In a simulated ECOA adverse action pipeline (10,000 loan applications, 5 correlated income features at $\rho = 0.7$, 25 XGBoost models), 43.2\% of denied applicants receive a different ``top reason for denial'' depending on which equivalent model is queried. \DASH{} ensemble averaging resolves this entirely: 100\% stable adverse action reasons after consensus.
...
\cite{hooker2021unrestricted} show that unrestricted permutation-based importance forces extrapolation into out-of-distribution regions, a complementary failure mode to our stability impossibility.
...
In a survey of 77 public datasets, 92\% have feature pairs with $|\rho| > 0.5$ and 68\% exhibit attribution instability (93\% for $P \\geq 20$)---the impossibility is a practical reality. Our theorem proves this is a ``known and foreseeable circumstance'' under EU AI Act Art.~13(3)(b)(ii) \cite{euaiact2024}.
...
\begin{itemize}
\item \textbf{Zero behavioral axioms:} The core impossibility (Theorem~\ref{thm:impossibility}) depends only on the Rashomon property as a hypothesis---no domain-specific axioms whatsoever.
\item \textbf{Nine axioms:} The GBDT impossibility, the ratio bound $1/(1{-}\rho^2)$, and the equity violation all hold from per-model proportionality (each model has its own constant $c_f$). The global proportionality axiom ($\exists\, c > 0$ uniform across models) is \emph{not required}---$c_f$ cancels in every ratio.
\item \textbf{Sixteen axioms:} The full set is needed only for DASH convergence (cross-model consistency), Spearman bounds (cross-model attribution sums), and the query complexity scaling constant.
\end{itemize}
