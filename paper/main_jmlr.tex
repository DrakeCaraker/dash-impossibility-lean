...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (227 theorems from 14 domain-specific + 2 query-complexity axioms, 0 \texttt{sorry})---to our knowledge, the first formally verified impossibility in explainable AI.
...
The proof is mechanically verified in Lean~4 using Mathlib: 227 type-checked theorems and lemmas across 42 files (75 with multi-step proofs of ${\geq}5$ tactic lines; the remainder are definitions, wrappers, and single-step applications; from 14 domain-specific + 2 query-complexity axioms; 0 \texttt{sorry}).
...
\begin{table}[h]
\centering
\caption{Lean file cross-reference (42 files, 227 theorems, 16 axioms).}
...
\paragraph{Robustness conjecture.}
We conjecture that the flip rate prediction $\Phi(-\text{SNR})$ is Lipschitz-continuous in the correlation matrix: if $\|\hat\Sigma - \Sigma\|_F < \varepsilon$, the predicted flip rates change by $O(\varepsilon/\sigma)$. Simulation on Breast Cancer supports this, but a formal proof is open.
