...
With deterministic training, XGBoost produces identical models across seeds---no Rashomon set, no instability. With the recommended stochastic setting (\texttt{subsample=0.8}), each seed produces a different near-optimal model, and feature rankings flip. The impossibility is about the stochastic (recommended, regularized) setting, not a bug in the algorithm.
...
Adversarial configs & Worst-case grid search (108 configs) & \texttt{adversarial\_max\_instability.py} \\
...
Method comparison & SHAP, gain, permutation all unstable & \texttt{attribution\_method\_comparison.py} \\
SAGE approximation & Alternative methods also unstable & \texttt{sage\_comparison.py} \\
DASH breakdown & Robustness to contaminated ensembles & \texttt{dash\_breakdown\_point.py} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Quickstart: Using \DASH{} in Practice}
\label{sec:quickstart}

The \DASH{} workflow requires only standard tools (XGBoost + SHAP):

\begin{verbatim}
import xgboost as xgb, shap, numpy as np

# Step 1: Train M models with different seeds
M = 25
models = [xgb.XGBRegressor(subsample=0.8, random_state=i).fit(X, y)
          for i in range(M)]

# Step 2: Compute SHAP for each model
shap_values = [shap.TreeExplainer(m).shap_values(X_test) for m in models]

# Step 3: DASH consensus = mean |SHAP| across models
consensus = np.mean([np.mean(np.abs(sv), axis=0) for sv in shap_values], axis=0)

# Step 4: Rank features by consensus importance
ranking = np.argsort(-consensus)
print("DASH feature ranking:", ranking)
\end{verbatim}

For the full F5$\to$F1$\to$\DASH{} diagnostic workflow, see the companion library at \url{https://github.com/DrakeCaraker/dash-shap}.
