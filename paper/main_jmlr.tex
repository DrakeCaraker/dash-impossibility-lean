...
Feature instability changes the explanations real people receive. On German Credit (1,000 loan applications, 30 models per condition): under standard regularization (\texttt{subsample=0.8}), \textbf{45\%} of applicants receive a different ``most important feature'' category across equivalent models (46\% for LightGBM, 35\% for Random Forest). The deterministic control (0\% for boosted models) confirms the instability is from model multiplicity. A single applicant can receive 6 different top-feature labels across 30 seeds. \DASH{} consensus at $M = 25$ achieves 92\% agreement.
