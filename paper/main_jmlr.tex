...
The impossibility is quantitative: the attribution ratio diverges as $1/(1{-}\rho^2)$ for gradient boosting, is infinite for Lasso, and converges for random forests. At the component level, the $G$-invariant projection (orbit averaging over per-layer permutation symmetry) lifts agreement from $\rho = 0.277$ to $0.873$ on GPT-2 (Cohen's $d = 19.2$) and from $0.55$ to $> 0.97$ on TinyStories. The achievable design space consists of exactly two families; the stable projection (\DASH{} for features, $G$-invariant for circuits) is Pareto-optimal on the stable branch. The framework is verified in Lean~4 (358 theorems, 6 axioms, 0~\texttt{sorry}; 139 with multi-step proofs of ${\geq}5$ tactic lines)---to our knowledge, the first formally verified impossibility in explainable AI.
\end{abstract}
...
Ten TopK Sparse Autoencoders ($d_{\text{SAE}}{=}6{,}144$, $k{=}48$) trained on layer-6 activations of a single frozen GPT-2-small model show that learned dictionary features are not reproducible: mean max cosine $= 0.394 \pm 0.002$ ($2.6\times$ random but not reliable), and importance Spearman after optimal Hungarian matching $= 0.500$ (chance-level). Under our experimental conditions (TopK, layer 6, 50K steps), SAE features do not provide a reproducible basis for interpretability. This is an empirical finding; larger dictionaries, different architectures, or better matching may improve stability.
...
The formalization comprises 58 Lean~4 files with 6 axioms and 358 type-checked theorems and lemmas (0~\texttt{sorry}). Of these, 139 require multi-line proofs ($\geq$5 tactic lines); the remainder are definitions, wrappers, and single-step applications.
...
Of 358 type-checked declarations: 139 require multi-step proofs (${\geq}5$ tactic lines), 21 require ${\geq}10$ lines, and 5 require ${\geq}20$ lines. The five deepest proofs are listed below.
...
The Bayes-optimality half of the Design Space is proved but not in Lean (requires measure-theoretic decision theory). Deriving split-count axioms algorithmically from the TreeSHAP algorithm and proving the proportionality axiom for general tree depths remain open. The generalization from $\rho > 0$ to $I(X_j; X_k) > 0$ is now complete (\texttt{MutualInformation.lean}): mutual information is the exact boundary between possible and impossible ranking. Whether SAE features can be made reproducible via better training procedures or matching algorithms is an empirical question (Section~\ref{sec:sae}). Scaling component-level experiments beyond 124M parameters would strengthen the generality claim. A reference implementation is available at \url{https://github.com/DrakeCaraker/dash-shap}.
