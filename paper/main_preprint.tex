...
\DASH{} (\textbf{D}iversified \textbf{A}ggregation of \textbf{SH}AP) ensemble averaging is provably Pareto-optimal among unbiased aggregations on the stable branch, achieving the Cauchy--Schwarz lower bound (Titu's lemma; Cram\'er--Rao in the parametric case) with a tight ensemble size formula.
...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (352 theorems from 6 axioms, 0 \texttt{sorry})---to our knowledge, the first formally verified impossibility in explainable AI.
\end{abstract}
...
The proof is mechanically verified in Lean~4 using Mathlib: 352 type-checked theorems across 58 files from 6 axioms with 0 \texttt{sorry}.
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (352 theorems from 6 axioms, 0 \texttt{sorry}), publicly available at \url{https://github.com/DrakeCaraker/dash-impossibility-lean} (DOI: \url{https://doi.org/10.5281/zenodo.19468379}).
\end{enumerate}
...
Our result is the first to simultaneously address cross-model \emph{stability} as an impossibility, give quantitative architecture-discriminating bounds, and provide a constructive resolution with proved optimality (\DASH{}).
...
\citet{nipkow2009social} formalized Arrow's theorem in Isabelle/HOL; \citet{zhang2026statistical} formalized statistical learning theory in Lean~4. Ours is the first formally verified impossibility result in explainable AI.
...
The core impossibility is Lean-verified with zero axiom dependencies. The impossibility and ratio bound $1/(1{-}\rho^2)$ require only a subset of the 6 axioms---the global proportionality constant is unnecessary ($c_f$ cancels in ratios; verified by \texttt{\#print axioms}). The full formalization comprises 352 theorems with 0 \texttt{sorry} across 58 files (supplement).
...
\item \textbf{Experiments.}
  \begin{itemize}
    \item Code: \url{https://github.com/DrakeCaraker/dash-impossibility-lean} (DOI: \url{https://doi.org/10.5281/zenodo.19468379})
