...
In a survey of 37 public datasets, 60\% are affected.
...
\DASH{} ensemble averaging is provably Pareto-optimal on the stable branch.
...
The proof is mechanically verified in Lean~4 using Mathlib: 44 type-checked declarations (35 theorems + 9 lemmas, of which 26 are substantive proofs from 15 explicit axioms) with no unresolved goals.
...
In a survey of 37 public datasets (supplement), 92\% have feature pairs with $|\rho| > 0.5$ and 60\% exhibit attribution instability (ranking flip rate $> 10\%$) --- the impossibility is not a theoretical curiosity but a practical reality affecting the majority of real-world data.
...
The expected fraction of non-replicable pairwise rankings is bounded below by $R/2$, where $R$ is the \emph{Rashomon coefficient}---the fraction of feature pairs that are within collinear groups---computable from the correlation matrix alone without training any models (supplement).
