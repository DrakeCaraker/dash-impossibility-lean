...
\begin{abstract}
Feature rankings from SHAP change when you retrain the model---in 68\% of 77 public datasets (under the standard workflow; model variation accounts for ${\sim}63\%$ of the total), the ``most important'' feature flips with the random seed. We prove this is not an engineering problem but a mathematical impossibility: no feature ranking can be simultaneously faithful (reflecting the model), stable (consistent across retrains), and complete (ordering all features) when features are collinear with similar true importance.
The impossibility is quantitative, with architecture-discriminating bounds: the attribution ratio diverges as $1/(1{-}\rho^2)$ for gradient boosting and is infinite for Lasso, while random forests converge.
The achievable design space consists of exactly two families; \DASH{} ensemble averaging is Pareto-optimal among unbiased aggregations on the stable branch, achieving equitable attributions with Cauchy--Schwarz-optimal variance.
For binary attribution questions (SHAP sign, feature selection, counterfactual direction), the impossibility is strictly stronger: faithful + stable alone is impossible (the bilemma).
A theory-derived nonparametric diagnostic (minority fraction) predicts instability with Spearman $0.92$--$0.98$, outperforming the Gaussian formula on real data ($0.96$ vs.\ $0.46$).
The impossibility, design space, and resolution are mechanically verified in Lean~4 (357 theorems, 6 axioms, 0~\texttt{sorry}; 139 with multi-step proofs of ${\geq}5$ tactic lines)---the first formally verified impossibility result in explainable AI.
\end{abstract}
...
The proof is mechanically verified in Lean~4 using Mathlib: 357 type-checked theorems across 58 files from 6 axioms with 0 \texttt{sorry}.
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (357 theorems from 6 axioms, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}.
\end{enumerate}

The core impossibility proof is deliberately simple---a four-line contradiction from the Rashomon property.
The contribution is in identifying the right abstraction (the Rashomon property) and characterizing the complete design space, not in proof complexity.
...
When desirable properties conflict, relaxing completeness (accepting ties) restores consistency.
...
Our relaxation paths \emph{converge} (supplement), reflecting the DGP symmetry.
...
\end{theorem}
\begin{proof}
Binary $H$ has no neutral element: every value contradicts the other. Faithful forces $E(\theta) = \mathrm{explain}(\theta)$ (there's nowhere else to go), making $E$ decisive. The trilemma gives $\bot$.
\end{proof}

The tightness classification determines which problems admit resolutions: $F{+}S$ is achievable iff $H$ has a neutral element (Lean: \texttt{tightness\_dichotomy}). Rankings have ties (neutral); SHAP signs do not. All three binary instances (\texttt{shap\_sign\_bilemma}, \texttt{feature\_selection\_bilemma}, \texttt{counterfactual\_bilemma}) use constructive inductive types with zero axioms.

\paragraph{Nonparametric flip predictor.}
The theory yields a practical diagnostic. An explanation system has \emph{coverage conflict} if every candidate explanation clashes with some model's native explanation (\texttt{hasCoverageConflict}). The \emph{minority fraction} $\mathrm{MF}_j = \min(\text{count}(\text{SHAP}_j > 0), \text{count}(\text{SHAP}_j < 0))/M$ is the empirical coverage conflict strength. It predicts flip rates with Spearman $0.92$--$0.98$ across all datasets, outperforming the Gaussian formula ($0.46$--$0.89$); the advantage is decisive on California Housing ($0.96$ vs.\ $0.46$). The all-or-nothing theorem predicts bimodal flip rate distributions; confirmed by the Hartigan dip test ($p < 0.002$ for $\rho \geq 0.5$, control passes at $\rho = 0$). Workflow: Screen $\to$ Minority fraction $\to$ Z-test $\to$ DASH.
...
None address cross-model \emph{stability}, quantitative bounds, or constructive resolution.
\citet{laberge2023partial} find partial-order rankings across Rashomon sets; \citet{rudin2024amazing} advocate embracing model multiplicity---our theorem proves the partial order is a mathematical necessity.
\citet{chouldechova2017fair} and \citet{kleinberg2017inherent} prove fairness trilemmas; our trilemma is structurally analogous.
\citet{nipkow2009social} formalized Arrow in Isabelle/HOL; \citet{zhang2026statistical} formalized statistical learning in Lean~4; ours is the first formally verified impossibility in XAI.
Extended discussion in supplement.
...
In 68\% of 77 public datasets, attribution instability exceeds 10\% (supplement). Single-model SHAP audits for proxy discrimination are provably coin flips under collinearity. Our result establishes this as a ``known and foreseeable circumstance'' under EU AI Act Art.~13(3)(b)(ii)~\citep{euaiact2024}.

\paragraph{Classical multicollinearity vs.\ ranking impossibility.}
The classical concern is estimation variance ($\Var(\hat\beta_j - \hat\beta_k) \to \infty$ as $\rho \to 1$). Our result is qualitatively different: even with perfect estimates, the ranking of symmetric features is a coin flip because different Rashomon-set models rank them in opposite orders. A fixed seed produces a deterministic ranking; instability arises upon retraining---routine in regulated settings~\citep{damour2022underspecification}.

\paragraph{Extensions (supplement).}
The symmetric Bayes dichotomy proves the two-family structure for any symmetric decision problem; demonstrated across feature attribution, model selection, and causal discovery.
The Lean formalization (357 theorems, 6 axioms, 0~\texttt{sorry}) caught two logical inconsistencies and one type mismatch that survived informal review.
Loss landscape geometry: near-singular Fisher information creates ridges along which feature importance redistributes without changing model quality.
...
The core impossibility is Lean-verified with zero axiom dependencies. The formalization uses only 6 axioms (reduced from 16 by defining split counts, attribution, and the testing constant as definitions with derived properties). The impossibility and ratio bound $1/(1{-}\rho^2)$ require only 3 of 6 axioms; the proportionality constant is unnecessary for the ratio ($c$ cancels; verified by \texttt{\#print axioms}). The full formalization comprises 357 theorems with 0 \texttt{sorry} across 58 files (supplement).
