...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (128 theorems, 15 axioms, 0 \texttt{sorry})---to our knowledge, the first formally verified impossibility in explainable AI.
\end{abstract}
...
The proof is mechanically verified in Lean~4 using Mathlib: 128 type-checked theorems and lemmas (from 15 explicit axioms across 32 files) with no unresolved goals.
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (128 type-checked theorems from 15 axioms, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}. The axiom system is consistent: we construct an explicit model satisfying all 15 axioms simultaneously (supplement).
\end{enumerate}
...
The DASH equity result (Corollary~\ref{cor:equity}) follows from \texttt{attribution\_sum\_symmetric}, which is now a derived theorem (proved in \texttt{SymmetryDerive.lean} from the proportionality, split-count, and cross-group symmetry axioms).
The Design Space Theorem, DASH optimality (F2), and FIM impossibility (F3) are proved in the supplement; the impossibility of consensus-value-faithful methods is formalized in Lean (\texttt{strongly\_faithful\_impossible}; this proves a stronger property than the supplement's Step~3, which places ranking-faithful methods in Family~$\mathcal{A}$ with $U{=}1/2$ rather than deriving contradiction); the Bayes-optimality half (optimal unfaithful ranking assigns ties) requires probability theory beyond the current axiom system.
