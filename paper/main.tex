...
Feature attribution and model selection are both instances; we anticipate the technique applies to feature selection (Lasso instability), causal discovery (Markov equivalence), hyperparameter tuning (CV noise), and \emph{LLM interpretability}: in the supplement we show empirically that DistilBERT attention-based token importance rankings are unstable---88\% of adjacent token pairs have flip rates exceeding 10\% across model perturbations, confirming that the impossibility extends to transformer attention.
