...
The proof is mechanically verified in Lean~4 using Mathlib: 180 type-checked theorems and lemmas (from 17 explicit axioms across 36 files; two \texttt{sorry} remain for Gaussian CDF symmetry properties pending Mathlib infrastructure---no domain result depends on them).
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (180 type-checked theorems from 17 axioms, 2 \texttt{sorry} for Gaussian CDF symmetry only), publicly available at \url{[anonymized for review]}. The axiom system is consistent: we construct an explicit model satisfying all 14 domain-specific axioms simultaneously, both in Lean (\texttt{Consistency.lean}) and numerically (supplement).
\end{enumerate}
