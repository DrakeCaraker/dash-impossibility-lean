...
\paragraph{Generality of the pipeline.}
The Rashomon $\to$ impossibility $\to$ design space $\to$ resolution pipeline is not specific to feature attribution.
In the supplement, we apply the same pipeline to \emph{model selection}: when candidates have similar population loss, no selection rule can be simultaneously faithful (respect held-out performance), stable (independent of validation split), and complete (decide for every pair).
The resolution is the same: ensemble (cross-validate) rather than select.
We conjecture this \emph{two-families pattern}---instance-specific vs.\ population-averaged---holds generally for statistical decision problems under symmetric populations and finite-sample noise.
