...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (188 type-checked theorems from 18 axioms, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}. The axiom system is consistent: we construct an explicit model satisfying all 15 domain-specific axioms simultaneously, both in Lean (\texttt{Consistency.lean}) and numerically (supplement).
\end{enumerate}
...
The following are now \emph{fully formalized} in Lean (188 theorems, 0 \texttt{sorry}): the Design Space Theorem (all 4 steps, including exhaustiveness), the symmetric Bayes dichotomy (general theorem with orbit-cardinality bounds), all three impossibility instances (attribution, model selection, causal discovery), the conditional attribution impossibility (with escape condition), the Gaussian FIM impossibility and flip rate formula ($\Phi(0) = 1/2$ and $\Phi(-x) = 1 - \Phi(x)$ proved from Mathlib's \texttt{gaussianReal}), the fairness audit impossibility, the exact GBDT flip rate, DASH variance optimality, the Rashomon inevitability, and the unfaithfulness/path-convergence results.
