...
Second, model-specific bounds quantify the severity: under full signal capture, the gradient-boosted tree attribution ratio is $1/(1-\rho^2)$ (Lean-verified); for finite-depth trees, the empirically corrected $1/(1-\alpha\rho^2)$ with $\alpha \approx 2/\pi$ matches data ($R^2{=}0.89$); for Lasso the ratio is infinite; while random forests remain bounded at $O(1/\sqrt{T})$.
...
The proof is mechanically verified in Lean~4: 33 theorems and 9 technical lemmas (42 total, of which 24 are substantive proofs) type-checked from 15 axioms with no unresolved goals---to our knowledge, the first formally verified impossibility result in explainable AI.
\end{abstract}
