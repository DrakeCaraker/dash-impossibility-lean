...
Second, model-specific bounds quantify the severity: for gradient-boosted trees, the attribution ratio follows $1/(1-\alpha\rho^2)$ where $\alpha$ is the per-tree signal capture fraction ($\alpha \approx 0.62$ empirically, $R^2 = 0.96$); for Lasso, the ratio is infinite; while random forests remain bounded at $O(1/\sqrt{T})$.
...
Theorem~\ref{thm:ratio} assumes full signal capture ($\alpha{=}1$); for finite-depth trees, $\alpha < 1$ yields a corrected ratio $1/(1{-}\alpha\rho^2)$ with $\alpha \approx 0.62$ for stumps ($R^2{=}0.96$; Figure~\ref{fig:ratio}).
...
\caption{Attribution ratio vs.\ $\rho$. The corrected model $1/(1{-}\alpha\rho^2)$ with $\alpha{=}0.62$ (solid) fits the data ($R^2{=}0.96$); the idealized $1/(1{-}\rho^2)$ (dashed) is a theoretical upper bound.}
\label{fig:ratio}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.31\textwidth}
\centering
...
The corrected model $1/(1-\alpha\rho^2)$ with fitted $\alpha = 0.62$ matches the data closely ($R^2 = 0.96$), confirming both the functional form and the signal-capture interpretation.
...
The split-count axioms assume full signal capture ($\alpha = 1$); for finite-depth trees, $\alpha < 1$ and the corrected ratio $1/(1{-}\alpha\rho^2)$ matches empirical data ($\alpha \approx 0.62$, $R^2 = 0.96$).
