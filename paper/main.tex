...
\begin{abstract}
We characterize the complete design space for feature attribution under collinearity: exactly two families of methods exist---faithful-complete methods that are unstable (rankings flip up to 50\% of the time) and ensemble methods like \DASH{} that are stable but report ties for symmetric features---and no method lies outside this dichotomy.
The foundation is the \emph{Attribution Impossibility}: no feature ranking can be simultaneously faithful, stable, and complete when features are collinear, a result we prove in four lines from the Rashomon property.
