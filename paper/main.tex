...
In the supplement we prove the Attribution Design Space Theorem: the achievable set of (stability, unfaithfulness, completeness) triples consists of exactly two families, with \DASH{} Pareto-optimal on the ensemble branch.
Quantifying the information loss from \DASH{} averaging (bits lost as a function of $M$ and $\rho$), deriving split-count axioms algorithmically in Lean~4, formalizing the Design Space Theorem in Lean, and generalizing from $\rho > 0$ to $I(X_j; X_k) > 0$ remain open.
