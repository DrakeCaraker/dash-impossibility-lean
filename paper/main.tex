...
For \GBDT{}, this ratio is $1/(1-\rho^2)$, diverging as $\rho \to 1$; for Lasso it is infinite at any $\rho > 0$; for neural networks it is unbounded in depth; and for random forests it is $O(1/\sqrt{T})$---converging rather than diverging.
...
The proof is mechanically verified in Lean~4 using Mathlib: 28 substantive theorems with no unresolved goals, plus one stated variance bound awaiting Mathlib infrastructure.
...
Our 29-theorem Lean~4 formalization is the first formally verified result in explainable AI.
...
The split-count axioms represent leading-order behavior of idealized greedy splitting; empirical ratios are lower (Figure~\ref{fig:ratio}), consistent with finite depth and learning-rate damping. Crucially, the qualitative prediction---ratio $> 1$, increasing with $\rho$---holds across all tested configurations, confirming the impossibility is structural.
...
\begin{enumerate}
\item \textbf{Claims.} All theoretical claims are formally verified by the Lean~4 type checker (28 substantive theorems, 0 \texttt{sorry}). Empirical claims are backed by experiments over 50 random seeds. [Yes]
