...
The entire proof---29 substantive theorems verified by the Lean~4 type checker with no unresolved goals---is mechanically verified in Lean~4, constituting the first formally verified impossibility result in explainable AI.
\end{abstract}
...
The proof is mechanically verified in Lean~4 using Mathlib: 29 substantive theorems with no unresolved goals, including the variance convergence bound for consensus attributions.
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: The entire argument is formalized in Lean~4 (29 substantive theorems, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}.
\end{enumerate}
...
For any model class satisfying the Rashomon property under within-group correlation $\rho > 0$, the achievable set of (stability~$S$, unfaithfulness~$U$, completeness~$C$) triples, over all methods operating on per-model attributions, is the union of exactly two families:
\begin{itemize}
...
\begin{enumerate}
\item \textbf{Claims.} All theoretical claims are formally verified by the Lean~4 type checker (29 substantive theorems, 0 \texttt{sorry}). Empirical claims are backed by experiments over 50 random seeds. [Yes]
