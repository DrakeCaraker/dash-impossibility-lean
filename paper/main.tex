...
These results are instances of a general \emph{symmetric Bayes dichotomy}: any statistical decision problem with symmetric alternatives admits exactly two operating modes, demonstrated here for both feature attribution and model selection.
The framework is mechanically verified in Lean~4 (42 declarations, 15 axioms, 0 \texttt{sorry})---to our knowledge, the first formally verified impossibility in explainable AI.
\end{abstract}
...
The proof is mechanically verified in Lean~4 using Mathlib: 42 type-checked declarations (33 theorems + 9 lemmas, of which 24 are substantive proofs from 15 explicit axioms) with no unresolved goals.
...
\begin{enumerate}
    ...
    \item \textbf{The symmetric Bayes dichotomy} (supplement): a general proof technique showing that ANY decision problem with a symmetric population has exactly two operating modes. Feature attribution and model selection are instances; the technique is reusable.
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (42 type-checked declarations from 15 axioms, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}.
\end{enumerate}
...
Feature attribution and model selection are both instances; we anticipate the technique applies to feature selection (Lasso instability), causal discovery (Markov equivalence), hyperparameter tuning (CV noise), and potentially \emph{LLM interpretability}: in transformer models, correlated token representations (nearby words, repeated entities) induce the same Rashomon structure for attention-based attribution, suggesting that token-level explanations of large language models face an analogous impossibility.
...
The Design Space Theorem, DASH optimality (F2), and FIM impossibility (F3) are proved in the supplement but not in Lean.
...
Formalizing the Design Space Theorem exhaustiveness proof in Lean, deriving split-count axioms algorithmically, proving the proportionality axiom for TreeSHAP, and generalizing from $\rho > 0$ to $I(X_j; X_k) > 0$ remain open.
...
\begin{enumerate}
\item \textbf{Claims.} All theoretical claims are type-checked by Lean~4 (42 declarations from 15 axioms, 0 \texttt{sorry}). The core impossibility (Theorem~\ref{thm:impossibility}) has zero axiom dependencies. Empirical claims are backed by experiments over 50 random seeds. [Yes]
