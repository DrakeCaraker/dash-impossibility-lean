...
The proof is mechanically verified in Lean~4 using Mathlib: 203 type-checked theorems across 40 files from 16 axioms with 0 \texttt{sorry}.
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (203 theorems from 16 axioms, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}.
\end{enumerate}
...
(In the Lean~4 formalization, two Spearman bounds exist: a fully derived bound $\rho_S \leq 1 - 3(m{-}1)^2/(P^3{-}P)$ and a tighter axiomatized bound $\rho_S \leq 1 - m^3/P^3$. The gap is a combinatorial counting argument---the effect of transposition on midranks---not a domain-specific assumption.)
...
The core impossibility is Lean-verified with zero axiom dependencies. The full formalization comprises 203 theorems from 16 axioms with 0 \texttt{sorry} across 40 files (supplement).
