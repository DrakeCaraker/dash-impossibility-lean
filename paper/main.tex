...
The design space has exactly two modes.
\emph{Mode~A}: rank features using a single model---faithful and complete, but unstable (within-group rankings flip with 50\% probability).
\emph{Mode~B}: average across an ensemble (\DASH{})---stable with variance $O(1/M)$, but reporting ties for symmetric features.
\DASH{} is provably Pareto-optimal: no method achieves higher stability without sacrificing faithfulness.
We quantify the severity across architectures: for gradient-boosted trees the attribution ratio is $1/(1{-}\rho^2)$ (diverging with correlation); for Lasso it is infinite; for random forests it converges to~1.
The entire framework---impossibility, design space characterization, and resolution---is mechanically verified in Lean~4, to our knowledge the first formally verified impossibility result in explainable AI.
\end{abstract}
...
Our main result is the \emph{Attribution Design Space Theorem} (\S\ref{sec:design-space}): a complete characterization of what is achievable when explaining models with collinear features.
...
Feature attribution and model selection are both instances; we anticipate the technique applies to feature selection (Lasso instability), causal discovery (Markov equivalence), and hyperparameter tuning (CV noise).
