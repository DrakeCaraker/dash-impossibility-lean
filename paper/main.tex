...
\begin{abstract}
No feature ranking can be simultaneously faithful, stable, and complete when features are correlated---an impossibility we prove in four lines from the Rashomon property, then quantify, resolve, and machine-verify in Lean~4.
For $m{=}2$ collinear features, the ranking is literally a coin flip: retraining with a different seed inverts the importance ordering 50\% of the time.
...
Train a gradient-boosted model on your data. Compute SHAP values. Retrain with a different random seed. The `most important feature' changes. In a survey of 37 public datasets, approximately 60\% exhibit this instability. Most practitioners assume this is an engineering problem---insufficient data, poor hyperparameters, or a software bug. We prove it is none of these. It is a mathematical impossibility.
...
The proof is mechanically verified in Lean~4 using Mathlib: 188 type-checked theorems across 36 files from 18 axioms with 0 \texttt{sorry}.
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (188 theorems from 18 axioms, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}.
\end{enumerate}

The core impossibility proof is a four-line contradiction from the Rashomon property; the quantitative depth comes from architecture-specific bounds (\S\ref{sec:bounds}) and the Pareto optimality proof (supplement).
...
The argument parallels Arrow's impossibility \citep{arrow1951social}: faithfulness $\leftrightarrow$ Pareto condition, stability $\leftrightarrow$ IIA, completeness $\leftrightarrow$ totality.
The resolution is also parallel: relaxing completeness (allowing ties) circumvents the impossibility.
...
\begin{itemize}
    \item \textbf{Family $\mathcal{A}$ (single-model):}
      $S \leq 1 - m^3/P^3$, $U = 1/2$, $C = \text{complete}$.
...
Our result is the first to address cross-model \emph{stability}, give quantitative architecture-dependent bounds, and provide a constructive resolution (\DASH{}).
...
\citet{nipkow2009social} formalized Arrow's theorem in Isabelle/HOL; \citet{zhang2026statistical} formalized statistical learning theory in Lean~4. Ours is the first formally verified result in explainable AI.
...
In a survey of 37 public datasets (supplement), approximately 60\% exhibit attribution instability (conservative lower bound; survey power 32\%).
...
The core impossibility is Lean-verified with zero axiom dependencies. The full formalization comprises 188 theorems from 18 axioms with 0 \texttt{sorry} across 36 files (supplement).
