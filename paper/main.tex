...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (188 theorems from 15 domain-specific axioms + 3 axioms for query complexity, 2 \texttt{sorry} for Gaussian CDF symmetry pending Mathlib)---to our knowledge, the first formally verified impossibility in explainable AI.
\end{abstract}
...
The proof is mechanically verified in Lean~4 using Mathlib: 188 type-checked theorems and lemmas across 36 files (75 with multi-step proofs of ${\geq}5$ tactic lines; the remainder are definitions, wrappers, and single-step applications; from 15 domain-specific + 3 query-complexity axioms; 2 \texttt{sorry} for Gaussian CDF symmetry pending Mathlib).
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (188 type-checked theorems from 18 axioms, 2 \texttt{sorry} for Gaussian CDF symmetry only), publicly available at \url{[anonymized for review]}. The axiom system is consistent: we construct an explicit model satisfying all 15 domain-specific axioms simultaneously, both in Lean (\texttt{Consistency.lean}) and numerically (supplement).
\end{enumerate}
...
The quantitative depth comes from the architecture-specific bounds (\S\ref{sec:bounds}) and the Pareto optimality proof (supplement).
...
Lasso is an iterative optimizer (Definition~\ref{def:iterative}) with $d(f) =$ the selected feature; the dominance condition holds with infinite margin.
Surjectivity holds because regularization-path tie-breaking and data perturbation can make any feature the selected one (the elastic net homotopy ensures every feature is selected for some $\lambda$).
The attribution ratio is therefore infinite at \emph{any} $\rho > 0$---the most severe possible violation.
The impossibility applies to any model class with hard feature selection, including stepwise regression and $\ell_0$-penalized methods.
...
A single-model screening variant using per-tree split frequencies achieves 94\% precision on Breast Cancer (supplement \S F5; implementation-specific calibration required).
The instability and diagnostics generalize to LightGBM and CatBoost ($|r| > 0.89$ for all three implementations; supplement) and to non-SHAP attributions: permutation importance shows even higher instability (91\% of pairs vs.\ 41\% for SHAP), confirming the phenomenon is about collinearity, not the attribution algorithm.
The instability extends beyond tree models: on the same Breast Cancer dataset, 20 neural networks (MLPRegressor with KernelSHAP) produce 380/435 (87\%) unstable pairs with F1 diagnostic $r = -0.871$ (supplement), confirming the impossibility is about collinearity, not the model class.
SHAP's efficiency axiom induces negative within-model correlation among collinear features' attributions (amplification factor $m/(m{-}1)$ for groups of size $m$; supplement), but this within-model effect does not increase across-model flip rates; the lower instability of SHAP relative to permutation importance is driven by coalition-averaging reducing the marginal variance $\sigma^2$.
...
A method subject to both (linear SHAP on collinear features) faces the intersection of constraints.
...
Our theorem shows the partial order is not a pragmatic choice but a mathematical necessity: completeness is formally incompatible with faithfulness and stability.
Concretely, we add four things Laberge et al.\ do not provide: (i)~a formal impossibility proof (they observe partial orders empirically; we prove they are \emph{unavoidable}), (ii)~quantitative architecture-discriminating bounds, (iii)~a provably optimal resolution (\DASH{}) with tight ensemble size formulas, and (iv)~machine-verified formalization.
The qualitative intuition that correlated features have unstable rankings is practitioner folklore; our contribution is making it \emph{precise} (exactly two families), \emph{quantitative} (architecture-discriminating bounds), and \emph{machine-verified}.
...
The equicorrelation assumption (all within-group pairs share the same $\rho$) simplifies the axioms; the Rashomon property holds pairwise---any pair $(j,k)$ with $|\rho_{jk}| > 0$ and similar importance admits models ranking them in opposite orders, regardless of other features' correlations.
The F1 and F5 diagnostics operate on individual pairs and do not require equicorrelation.
Detecting instability is itself computationally hard: $\Omega(\sigma^2_{jk}/\Delta^2_{jk})$ model-training queries are necessary for certification (supplement), and the $Z$-test diagnostic achieves this rate to within a logarithmic factor.
Our stability definition (ranking is model-independent) is all-or-nothing, following Arrow's use of all-or-nothing IIA.
Under a weaker $\varepsilon$-stability notion (ranking agrees with $\geq 1{-}\varepsilon$ fraction of models), the impossibility extends: no faithful, complete ranking can be $\varepsilon$-stable with $\varepsilon < 1/2$ for within-group pairs under DGP symmetry (supplement, unfaithfulness $= 1/2$).
...
Switching to conditional SHAP \citep{janzing2020feature} does not universally resolve the instability: in the supplement we prove the impossibility extends to conditional attributions whenever features have equal causal effects ($\beta_j = \beta_k$) and symmetric causal position (the causal graph preserves the symmetry).
The precise escape condition is $\beta_j \neq \beta_k$: when causal effects differ, conditional SHAP can resolve the pair by appealing to causal structure---for $\rho = 0.9$, a causal effect difference $|\Delta\beta| \geq 0.5$ eliminates instability (supplement).
This connects to the causal feature relevance framework of \citet{janzing2020feature}: causal structure can break the symmetry driving the impossibility, but only when it provides genuine asymmetry.
When features are causally interchangeable (e.g., multiple measurements of the same quantity), no attribution method---marginal or conditional---can produce stable complete rankings.
...
For batch pipelines (daily or weekly model refreshes), training $M{=}25$ models is feasible---many teams already train multiple models for hyperparameter search.
For real-time SHAP explanations, $M{=}25$ is prohibitive; use the F5 single-model diagnostic to flag unstable pairs, then report affected features with an instability caveat rather than ensemble-averaging.
For high-dimensional feature spaces ($P > 500$), the pairwise F1 diagnostic scales as $O(P^2)$; the correlation-based grouping step (identifying correlated groups from the correlation matrix) prunes this to $O(G \cdot m^2)$ where $G$ is the number of groups and $m$ their typical size.
At $P{=}500$ with 20 models, the full diagnostic workflow completes in under 2.5~minutes on commodity hardware (supplement).
The equicorrelation assumption simplifies the axioms but is not required for the diagnostics: F1 and F5 operate on observed attribution arrays and make no distributional assumptions.
The proportionality axiom (CV $\approx$ 0.35--0.66 depending on tree depth; supplement) affects the quantitative ratio $1/(1{-}\rho^2)$, not the qualitative impossibility or the diagnostics.

\paragraph{Broader implications.}
In a survey of 37 public datasets (supplement), 92\% have feature pairs with $|\rho| > 0.5$ and approximately 60\% exhibit attribution instability (ranking flip rate $> 10\%$; conservative lower bound---survey power 32\% at the 10\% threshold) --- the impossibility is not a theoretical curiosity but a practical reality affecting the majority of real-world data.
...
The low survey power (32\%) means the true prevalence of instability is likely higher than 60\%; our theorem establishes that this instability is not a statistical accident but a mathematical inevitability under collinearity.
...
In each case, \DASH{} provides a principled mitigation by reporting honest indistinguishability rather than arbitrary distinctions.
...
The impossibility characterizes the \emph{Rashomon set}---the population of models that could have been trained---not a single deployed model.
A fixed pipeline with a fixed seed produces a deterministic, reproducible ranking.
The instability arises when: (a)~the model is retrained (data refresh, library update, hyperparameter sweep), (b)~the explanation is audited by a different party (different seed), or (c)~the model is compared to a successor version.
In regulated settings, all three are routine: model risk management under SR~11-7 requires periodic revalidation, and fairness audits under ECOA involve independent testing.
The impossibility guarantees that these standard practices will produce contradictory explanations for collinear features.
For a single frozen deployment, the ranking is correct \emph{for that model}---but cannot be interpreted as a property of the data or the model class.
If a practitioner reports SHAP values with confidence intervals rather than rankings (e.g., $\varphi_j = 0.42 \pm 0.15$), the impossibility does not apply directly: the interval conveys that $\varphi_j$ and $\varphi_k$ are indistinguishable.
The impossibility is specifically about forcing continuous attributions into discrete rankings; confidence-interval reporting is an alternative to \DASH{} ties for communicating within-group indistinguishability.
...
The GBDT ratio, Spearman bounds, and DASH equity are Lean-derived from 15 domain-specific axioms.
The query complexity lower bound ($\Omega(\sigma^2/\Delta^2)$) is axiom-derived from Le~Cam's testing bound (3 axioms, axiomatized from Tsybakov 2009; not derived from Mathlib).
The following are now \emph{fully formalized} in Lean (188 theorems, 0 domain-level \texttt{sorry}): the Design Space Theorem (all 4 steps, including exhaustiveness), the symmetric Bayes dichotomy (general theorem with orbit-cardinality bounds), all three impossibility instances (attribution, model selection, causal discovery), the conditional attribution impossibility (with escape condition), the Gaussian FIM impossibility, the fairness audit impossibility, the exact GBDT flip rate, DASH variance optimality, the Rashomon inevitability, and the unfaithfulness/path-convergence results.
Two \texttt{sorry} remain for Gaussian CDF symmetry properties ($\Phi(0) = 1/2$, $\Phi(-x) = 1 - \Phi(x)$)---these are Mathlib infrastructure gaps, not domain assumptions; no domain-level theorem depends on them.
The F1 diagnostic characterization, information loss analysis, and 11-dataset validation are empirical.
Table~S1 in the supplement provides the full axiom inventory.

\paragraph{Open problems.}
The Bayes-optimality half of the Design Space (optimal unfaithful ranking assigns ties) is proved in the supplement but not in Lean (requires measure-theoretic decision theory).
Deriving split-count axioms algorithmically, proving the proportionality axiom for TreeSHAP, and generalizing from $\rho > 0$ to $I(X_j; X_k) > 0$ remain open.
A full-length JMLR version integrating the supplement is planned.
A reference implementation of the F5$\to$F1$\to$DASH pipeline is available at the companion repository; production deployment validation on enterprise ML platforms is in progress.
The Lean formalization provides machine-verified correctness of the theoretical framework; it does not replace empirical validation on specific datasets but guarantees the mathematical foundations are logically sound.
