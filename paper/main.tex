...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (49 declarations, 15 axioms, 0 \texttt{sorry})---to our knowledge, the first formally verified impossibility in explainable AI.
\end{abstract}
...
The proof is mechanically verified in Lean~4 using Mathlib: 49 type-checked declarations (40 theorems + 9 lemmas from 15 explicit axioms) with no unresolved goals.
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (49 type-checked declarations from 15 axioms, 0 \texttt{sorry}), publicly available at \url{[anonymized for review]}. The axiom system is consistent: we construct an explicit model satisfying all 15 axioms simultaneously (supplement).
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Claims.} Core theoretical claims are type-checked by Lean~4 (49 declarations from 15 axioms, 0 \texttt{sorry}). The core impossibility (Theorem~\ref{thm:impossibility}) has zero axiom dependencies; the Design Space Theorem and symmetric Bayes dichotomy are proved in the supplement but not in Lean. Empirical claims are backed by experiments over up to 50 random seeds. [Yes]
