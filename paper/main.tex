...
%% MOVED TO SUPPLEMENT: Theorem (Attribution Design Space) formal itemized statement
\begin{theorem}[Attribution Design Space --- informal]
\label{thm:design-space-main}
Under the Rashomon property, exactly two families of attribution methods exist: \textbf{Family~$\mathcal{A}$} (single-model: faithful and complete but unstable, with unfaithfulness $U{=}1/2$) and \textbf{Family~$\mathcal{B}$} (ensemble: stable with ties, $U{=}0$, stability $S_M = 1 - O(1/M)$). The ideal $(S{=}1, U{=}0, C{=}\text{complete})$ is infeasible. \DASH{} is Pareto-optimal on $\mathcal{B}$. The formal statement, Pareto optimality argument, and five corollaries are in the supplement.
\end{theorem}
