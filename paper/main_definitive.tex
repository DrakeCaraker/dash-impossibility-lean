...
\textbf{Component-level.} Ten GPT-2-small models trained from scratch on OpenWebText produce 32 distinct top-5 component rankings with 8 different \#1 components across seeds. Within-layer head flip rate $= 0.500$ [0.482, 0.517]---a coin flip, as predicted. The $G$-invariant projection (orbit averaging over per-layer permutation symmetry) lifts agreement from $\rho = 0.277$ to $\rho = 0.873$ (Cohen's $d = 19.2$). On the IOI circuit, raw agreement is $\rho = 0.158$ (near chance), within-layer flip $= 0.499$, and 32 distinct top-5 rankings appear---component-level rankings are a lottery. At TinyStories scale (4L/4H and 6L/8H), the $G$-invariant projection achieves $\rho > 0.97$, and all four layer-0 heads appear as \#1 across 10 seeds, realizing the full $S_4$ symmetric group. SAE learned features are non-reproducible by mathematical necessity (overcompleteness guarantees Rashomon): three architectures/conditions tested, none achieves reproducibility (best cosine $= 0.443$; ReLU is catastrophic at $0.136 \approx$ random).
...
\paragraph{PC resolution: the knockout result.}
We test the theoretical prediction by projecting SAE feature importance onto the principal components of layer-6 activations (computed from 400 random-token inputs on the base model---seed-invariant). For each SAE seed, we compute $w_j = \sum_i \text{imp}_i \cdot (d_i \cdot u_j)^2 / \|d_i\|^2$, measuring how much total importance each seed allocates to each PC direction.

\begin{table}[ht]
\centering
\caption{PC resolution for SAE features. Projecting importance onto data PCs produces near-perfect reproducibility ($\rho = 0.960$), surpassing the $G$-invariant projection for attention heads ($\rho = 0.873$). The question ``which data directions carry importance?'' has a seed-invariant answer even though ``which features are important?'' does not.}
...
\midrule
Raw SAE importance & $-0.002$ & 6{,}144 \\
Hungarian matching (prior) & 0.500 & 500 \\
$G$-invariant (attention heads) & 0.873 & 24 \\
\textbf{PC projection (SAE)} & \textbf{0.960} & \textbf{400} \\
\midrule
\multicolumn{3}{l}{\emph{By PC dimensionality:}} \\
\quad Top-10 PCs & 0.888 & 10 \\
\quad Top-50 PCs & 0.811 & 50 \\
\quad Top-100 PCs & 0.888 & 100 \\
\quad Top-200 PCs & 0.937 & 200 \\
\quad Top-400 PCs & 0.960 & 400 \\
\bottomrule
\end{tabular}
\end{table}

The result is definitive: SAE feature importance IS reproducible when projected onto a seed-invariant basis. The raw non-reproducibility ($\rho = -0.002$) is entirely null-space noise; the PC-projected signal ($\rho = 0.960$) is nearly perfectly stable. This validates the Explanation Capacity Theorem: the reproducible subspace exists, has dimension ${\sim}200$--$400$ (well below the theoretical ceiling of $d = 768$), and the data PCs provide the correct basis.

\paragraph{Why this works.}
The data PCs capture the principal directions of variation in the model's activations. Every SAE must cover these directions to reconstruct well. The \emph{specific features} used to cover each direction vary across seeds (null-space freedom), but the \emph{total importance allocated to each direction} is determined by the data distribution and the model's representations---both seed-invariant. The PC projection discards the arbitrary feature-level decomposition and retains the seed-invariant per-direction allocation.

This is precisely the Reynolds operator for the continuous symmetry group: averaging over all equivalent decompositions of importance into a given data direction. The data PCs provide the orbit structure that the spectral clustering failed to discover (Section~\ref{sec:gpt2-from-scratch}).

\begin{itemize}
  \item \textbf{Heads}: average importance within each layer $\to$ $L$-dimensional invariant ($L = 12$ for GPT-2). Group: $S_{12}$ per layer (known, finite).
  \item \textbf{SAE features}: project onto data PCs $\to$ ${\sim}200$--$400$-dimensional invariant. Group: null-space rotations (continuous, but PCs provide the invariant basis).
\end{itemize}
Both sacrifice feature-level granularity for stability---exactly the faithfulness-completeness tradeoff the Design Space Theorem predicts. The PC resolution is the SAE analogue of the $G$-invariant projection: it IS the Reynolds operator for the dictionary's symmetry group, computed via the data covariance rather than group enumeration.
