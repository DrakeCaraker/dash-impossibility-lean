...
We quantify the impossibility for four model classes, resolve it via SHAP (SHapley Additive exPlanations) ensemble averaging with \DASH{}, and machine-verify it with 315 Lean~4 theorems.
...
\noindent\textbf{Verification.} The framework is mechanically verified in Lean~4: 315 theorems from 16 axioms with 0 \texttt{sorry} across 56 files. The formalization caught two logical inconsistencies that survived informal review.
...
This document is the complete reference: impossibility theorem, quantitative bounds, constructive resolution, three Symmetric Bayes Dichotomy (SBD) instances, diagnostics, experiments across 11 datasets and 3 GBDT implementations, financial case studies, and a 315-theorem Lean~4 formalization.
...
\begin{sloppypar}
The proof is mechanically verified in Lean~4 using Mathlib: 315 type-checked theorems
and lemmas across 56 files (80 with multi-step proofs of ${\geq}5$ tactic lines; the
...
\begin{sloppypar}
The formalization comprises 56 Lean~4 files with 16 axioms and 315 type-checked
...
\begin{table}[t]
\centering
\caption{Lean file cross-reference (56 files, 315 theorems, 16 axioms).}
