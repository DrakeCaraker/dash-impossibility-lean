...
The F5 and F1 diagnostics developed here are formal hypothesis tests with controlled type~I error. The companion first-mover bias paper introduces complementary \emph{exploratory} diagnostics: FSI (Feature Stability Index, a per-feature coefficient of variation across models) and the IS~Plot (Importance-Stability scatter plot). The recommended combined workflow is: F5 (screen, 1 model) $\to$ F1 (confirm, 5 models) $\to$ \DASH{} (resolve, $M \geq 25$) $\to$ FSI/IS~Plot (audit).
...
\paragraph{F5 caveat.}
\begin{sloppypar}
The F5 split-frequency diagnostic requires access to per-tree split counts.
...
does not transfer the XGBoost-calibrated threshold. For CatBoost, the F1
multi-model diagnostic ($r = -0.892$) is recommended instead. LightGBM's feature
...
\begin{sloppypar}
The F1 diagnostic achieves $|r| > 0.8$ on 9/10 datasets with unstable pairs (all
...
improve this correlation. F5 precision is 94--100\% on small/clean datasets (Breast
...
This confirms F5 as a conservative \emph{screening} tool that works well for
moderate-$P$ datasets but should be supplemented by the full F1 test for
