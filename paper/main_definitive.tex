...
We prove this impossibility, quantify it for four model classes, resolve it via \DASH{} ensemble averaging, and machine-verify it with 260 Lean~4 theorems.
...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (260 theorems from 14 domain-specific + 2 query-complexity axioms, 0 \texttt{sorry})---to our knowledge, the first formally verified impossibility in explainable AI.
...
\textbf{Verification.} The framework is mechanically verified in Lean~4: 260 theorems from 16 axioms with 0 \texttt{sorry} across 46 files. The formalization caught two logical inconsistencies that survived informal review.
...
Train a gradient-boosted model on your data. Compute SHAP values. Retrain with a different random seed. The ``most important feature'' changes. In a survey of 77 public datasets, 68\% exhibit this instability. Most practitioners assume this is an engineering problem---insufficient data, poor hyperparameters, or a software bug. We prove it is none of these. It is a mathematical impossibility. For $m{=}2$ collinear features, ranking the pair is literally a coin flip: retraining with a different seed inverts the importance ordering 50\% of the time.

This document is the complete reference: impossibility theorem, quantitative bounds, constructive resolution, three SBD instances, diagnostics, experiments across 11 datasets and 3 GBDT implementations, financial case studies, and a 260-theorem Lean~4 formalization.
...
Our result plays the same role for explainability---instability under collinearity is not an engineering deficiency but a mathematical inevitability---with direct implications for the EU AI Act \cite{euaiact2024} (Art.~13(3)(b)(ii)), which requires disclosing ``known and foreseeable circumstances'' which may lead to risks to health, safety or fundamental rights.
...
The proof is mechanically verified in Lean~4 using Mathlib: 260 type-checked theorems and lemmas across 46 files (67 with multi-step proofs of ${\geq}5$ tactic lines; the remainder are definitions, wrappers, and single-step applications; from 14 domain-specific + 2 query-complexity axioms; 0 \texttt{sorry}).
...
\[
    \rho_S(f, f') \;\leq\; 1 - \frac{3(m-1)^2}{P^3 - P}.
\]
...
\emph{Relationship to ranking stability $S$.} The Design Space Theorem (\S\ref{sec:design-space}) uses the full-ranking stability $S$ (expected Spearman correlation). The pairwise stability $S_{jk}$ here is a per-pair component: high $S_{jk}$ for all between-group pairs is necessary (but not sufficient) for high $S$. Within-group pairs contribute to $S$ through their random ordering, which is why $S \leq 1 - 3(m{-}1)^2/(P^3{-}P)$ even when all between-group $S_{jk}$ are high.
\end{definition}
...
\begin{itemize}
    \item \textbf{Family $\mathcal{A}$ (single-model):} $S \leq 1 - 3(m{-}1)^2/(P^3{-}P)$, $U = 1/2$, $C = \text{complete}$. Any method faithful to an individual model falls here.
...
By the Rashomon property, for any within-group pair $(j,k)$, there exist models ranking them in opposite orders. By Theorem~\ref{thm:unfaithfulness-bound}, any faithful, complete ranking has $U = 1/2$ for symmetric pairs. Stability is bounded by $S \leq 1 - 3(m{-}1)^2/(P^3{-}P)$ from the Spearman bound (Lean-derived).
...
\midrule
1 & $\leq 1 - 3(m{-}1)^2/(P^3{-}P)$ & $1/2$ & Complete \\
...
The formalization comprises 46 Lean~4 files with 16 axioms and 260 type-checked theorems and lemmas (0 \texttt{sorry}). Of these, 67 require multi-line proofs ($\geq$5 tactic lines); 6 are definitional wrappers or single-step applications. The core impossibility (\texttt{attribution\_impossibility}) depends on zero axioms---only the Rashomon property as hypothesis. The DASH resolution (\texttt{consensus\_equity}) depends on the \texttt{attribution\_sum\_symmetric} theorem (derived from axioms); the variance convergence depends on the \texttt{consensus\_variance\_bound} theorem (also derived).
...
\begin{table}[t]
\centering
\caption{Lean file cross-reference (46 files, 260 theorems, 16 axioms).}
...
\cite{herren2023statistical} develop statistical inference for feature rankings under model multiplicity; our contribution is to prove that the multiplicity is unavoidable under collinearity, not merely empirically common.
\cite{krishna2022disagreement} document the ``disagreement problem'' in explainable ML---different explanation methods produce different outputs for the same model---complementary to our result that the \emph{same} method produces different outputs for different equivalent models.
...
In a survey of 77 public datasets, 92\% have feature pairs with $|\rho| > 0.5$ and approximately 60\% exhibit attribution instability---the impossibility is a practical reality. Our theorem proves this is a ``known and foreseeable circumstance'' under EU AI Act Art.~13(3)(b)(ii) \cite{euaiact2024}.
...
The following are fully formalized in Lean (260 theorems, 0 \texttt{sorry}, 46 files): the Design Space Theorem, the symmetric Bayes dichotomy, all three impossibility instances, the conditional attribution impossibility, the Gaussian FIM impossibility and flip rate formula, the fairness audit impossibility, the intersectional fairness compounding, the exact GBDT flip rate, DASH variance optimality, the Rashomon inevitability, the flip rate robustness (Lipschitz continuity in $\rho$), the mutual information generalization, and the unfaithfulness/path-convergence results. The F1 diagnostic characterization, information loss analysis, and multi-dataset validation are empirical.
