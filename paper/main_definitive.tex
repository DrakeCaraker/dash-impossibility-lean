...
We quantify the impossibility for four model classes, resolve it via SHAP (SHapley Additive exPlanations) ensemble averaging with \DASH{}, and machine-verify it with 322 Lean~4 theorems.
...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (322 theorems from 14 domain-specific + 2 query-complexity axioms, 0 \texttt{sorry}; 97 with multi-step proofs of ${\geq}5$ tactic lines; the remainder are definitions, wrappers, and single-step applications)---to our knowledge, the first formally verified impossibility in explainable AI.
...
\noindent\textbf{Verification.} The framework is mechanically verified in Lean~4: 322 theorems from 16 axioms with 0 \texttt{sorry} across 57 files. The formalization caught two logical inconsistencies that survived informal review.
...
This document is the complete reference: impossibility theorem, quantitative bounds, constructive resolution, three Symmetric Bayes Dichotomy (SBD) instances, diagnostics, experiments across 11 datasets and 3 GBDT implementations, financial case studies, and a 322-theorem Lean~4 formalization.
...
\begin{sloppypar}
The proof is mechanically verified in Lean~4 using Mathlib: 322 type-checked theorems
and lemmas across 57 files (80 with multi-step proofs of ${\geq}5$ tactic lines; the
...
\begin{sloppypar}
The formalization comprises 57 Lean~4 files with 16 axioms and 322 type-checked
...
\begin{table}[t]
\centering
\caption{Lean file cross-reference (57 files, 322 theorems, 16 axioms).}
