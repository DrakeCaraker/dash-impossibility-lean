...
This validates the core theoretical claim: \emph{resolution quality is proportional to knowledge of the symmetry group.} Data-driven methods (PCA) actively fail because they identify the symmetry noise as the dominant signal. The correct resolution must KNOW the group (or a proxy): for heads, average within layers; for SAE features, project onto the data's principal components (which are seed-invariant and form a natural basis for $V^G$).

\begin{itemize}
  \item \textbf{Heads}: average importance within each layer $\to$ $L$-dimensional invariant ($L = 12$ for GPT-2). Requires knowing the layer structure.
  \item \textbf{SAE features}: project onto data PCs $\to$ $d$-dimensional invariant ($d = 768$). Requires computing the data covariance (seed-invariant). Untested empirically (requires raw decoder weights not available locally).
\end{itemize}
Both sacrifice feature-level granularity for stability---exactly the faithfulness-completeness tradeoff the Design Space Theorem predicts. The open question for SAEs is whether the data-PC resolution achieves a useful level of reproducibility in practice.
