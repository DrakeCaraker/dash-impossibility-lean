...
Breast Cancer produces \textbf{24 distinct top-3 rankings across 50 seeds} under standard XGBoost configuration with recommended regularization (Table~\ref{tab:ranking-lottery}). The ``most common'' ranking appears in only 12\% of runs. Two random seeds agree only 4.2\% of the time for the top-3. For the top-5: 46 distinct rankings, 0.4\% agreement.
