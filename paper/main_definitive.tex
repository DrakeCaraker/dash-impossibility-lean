...
\begin{assumption}[Attribution non-degeneracy]
...
\begin{remark}
Non-degeneracy is empirically universal (verified across all experiments in this paper: no two models produce exactly equal attributions for collinear features) but formally unproved. A rigorous proof would require a transversality argument showing that the level set $\{\varphi_j(f) = \varphi_k(f)\}$ is a lower-dimensional manifold in seed space. The core impossibility (Theorem~\ref{thm:impossibility}) does not depend on this assumption---it takes the Rashomon property as a hypothesis. Theorem~\ref{thm:rashomon-inevitability} below uses non-degeneracy to show Rashomon is \emph{inevitable}, not merely possible.
\end{remark}

\begin{theorem}[Rashomon inevitability]
\label{thm:rashomon-inevitability}
Let $A$ be a stochastic, symmetric training algorithm ($A$ applied to a within-group-permuted dataset produces the permuted model in distribution) for a permutation-closed model class $\mathcal{F}$. Under the non-degeneracy assumption (Assumption~\ref{thm:non-degeneracy}), for any DGP with $\rho > 0$, the Rashomon property holds: for any $j, k$ in the same collinear group, there exist models $f, f'$ in the support of $A$ with $\varphi_j(f) > \varphi_k(f)$ and $\varphi_k(f') > \varphi_j(f')$.
\end{theorem}

\begin{proof}
By non-degeneracy, $\Pr[\varphi_j(f) \neq \varphi_k(f)] = 1$, so either $\varphi_j > \varphi_k$ or $\varphi_k > \varphi_j$ almost surely. By algorithmic symmetry, $\Pr[\varphi_j > \varphi_k] = \Pr[\varphi_k > \varphi_j]$. Since these probabilities sum to 1, each equals $1/2$. Both events have positive probability, so both are realized.
\end{proof}
