...
\begin{abstract}
No importance ranking---of input features or internal components---can be simultaneously faithful, stable, and complete when interchangeable elements exist under statistical dependence. We prove this as a single impossibility theorem operating at two levels: input-level (SHAP on collinear features) and component-level (activation patching on architecturally symmetric attention heads). The core proof requires zero domain axioms.
...
\textbf{Input-level.} On Breast Cancer Wisconsin, 50 retrains produce 24 distinct top-3 rankings (4.2\% agreement). On a 10{,}935-gene expression dataset, the \#1 biomarker alternates between TSPAN8 (80\% of seeds) and CEACAM5/CEA (20\%)---the drug target depends on the training seed. Under standard regularization, 45\% of German Credit applicants receive a different top explanation across equivalent models. In 68\% of 77 public datasets, attribution rankings are unstable.
...
The disagreement between ablation methods is itself an instance of the impossibility: different ``methods'' function as different ``seeds,'' producing different raw attributions from the same model. Weight zeroing measures the effect of removing a component's learned weights; mean ablation measures the effect of clamping its activations to the population mean. These are genuinely different attribution functions $\varphi$ applied to the same model $\theta$. Yet the orbit-averaged structure $v^G$ is invariant to this choice ($\rho_{G\text{-inv}} > 0.97$ for both methods at both scales). The $G$-invariant projection absorbs not only seed-to-seed variation but also method-to-method variation---exactly as the Reynolds operator framework predicts, since the symmetry group $G$ acts on the component indices regardless of how importance is measured.
