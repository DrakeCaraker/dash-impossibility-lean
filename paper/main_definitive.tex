...
The framework is mechanically verified in Lean~4 (357 theorems from 6 axioms, 0~\texttt{sorry}; 139 with multi-step proofs of ${\geq}5$ tactic lines)---the first formally verified impossibility in explainable AI. The formalization caught two logical inconsistencies and one type mismatch that survived informal review.
...
Seven pre-registered predictions derived from the theory are evaluated per configuration; all 7/7 pass for Configs~A and~B, while Config~C passes 4/7 (correctly predicted by the theory as a non-Rashomon regime).
