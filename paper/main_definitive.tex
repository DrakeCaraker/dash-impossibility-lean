...
\begin{abstract}
No feature ranking can be simultaneously faithful, stable, and complete when features are collinear with similar true importance. We prove this impossibility, characterize the complete design space (exactly two families of methods, no third option), and resolve it via \DASH{} (\textbf{D}iversified \textbf{A}ggregation for \textbf{S}table \textbf{H}ypotheses), which is provably Pareto-optimal among unbiased aggregations.

For binary attribution questions---SHAP sign, feature selection, counterfactual direction---the impossibility is strictly stronger: faithful + stable alone is impossible (the \emph{bilemma}). The bilemma has collapsed tightness, making it structurally more severe than the fairness impossibility of Chouldechova (2017): the only resolution is enrichment (adding neutral elements), not trade-off.

On Breast Cancer Wisconsin---the canonical SHAP tutorial dataset---50 retrains produce 24 distinct top-3 rankings (35 at 100 retrains); two random seeds agree only 4.2\% of the time. On a 10{,}935-gene colon-vs-kidney expression dataset, the \#1 biomarker gene alternates between TSPAN8 (invasion/metastasis, 80\% of seeds) and CEACAM5/CEA (immune evasion, 20\%)---the specific drug target depends on the training seed. Under standard regularization, 45\% of German Credit loan applicants receive a different ``most important feature'' category across equivalent models.

The impossibility is quantitative: the attribution ratio diverges as $1/(1{-}\rho^2)$ for gradient boosting, is infinite for Lasso, conditional for neural networks (87\% of feature pairs unstable), and converges for random forests. A theory-derived nonparametric diagnostic---the minority fraction---identifies unstable features in seven lines of code, outperforming the Gaussian flip formula by $2\times$ on real data (Spearman $0.96$ vs.\ $0.46$ on California Housing). The variance $\Var[\varphi_j]$ across the Rashomon set equals the minimum MSE any stable method achieves; \DASH{} saturates it exactly. The diagnostic is model-class universal: validated for XGBoost, LightGBM, Random Forest, Ridge, and LASSO.

In a survey of 77 public datasets, 68\% exhibit attribution instability. Switching to conditional SHAP does not escape the impossibility when features have equal causal effects. SHAP-based proxy discrimination audits conclude ``proxy reliance'' with probability exactly $1/2$ for collinear features; for $K$ protected attributes, joint audit correctness is $(1/2)^K$. Preliminary evidence extends the impossibility to mechanistic interpretability: 10 grokked transformers agree on only 36\% of the top-3 circuit components.

The framework is mechanically verified in Lean~4 (357 theorems from 6 axioms, 0~\texttt{sorry}; 139 with multi-step proofs of ${\geq}5$ tactic lines)---to our knowledge, the first formally verified impossibility in explainable AI. The formalization caught two logical inconsistencies and one type mismatch that survived informal review.
...
\begin{itemize}
\item \textbf{Axioms (human-conceived).} All 6 axioms were designed by the first author based on mathematical analysis of the GBDT attribution mechanism. No axiom was suggested by an AI system. The axiom reduction from 16 to 6 (defining splitCount, attribution, and testing\_constant as computable definitions) was a joint human/AI effort: the first author identified which axioms should be definitions, and Claude Code produced the Lean 4 syntax.

\item \textbf{Theorem statements (human-conceived).} All theorem statements were written or specified by the first author. The mathematical content (what to prove and why) is entirely human. Claude Code did not propose theorems or identify proof targets.

\item \textbf{Proof tactics (AI-assisted).} The initial formalization (49 theorems, 15 files) was developed with iterative Claude Code assistance over several weeks. The expansion from 49 to 357 theorems (58 files) was produced by dispatching targeted Lean-writing agents with detailed mathematical specifications. Approximately 80\% of tactic-mode proofs were initially drafted by Claude Code and then verified, corrected, or restructured by the first author.

\item \textbf{Verification (machine-checked).} Every proof was accepted only after passing \texttt{lake build} (the Lean 4 type-checker). No theorem in the final formalization relies on \texttt{sorry} or unverified assertions. The type-checker is the final arbiter of correctness, regardless of whether the proof was human-written or AI-drafted.

\item \textbf{Paper text (human-directed, AI-assisted).} The paper text was written by the first author with AI editing and drafting assistance. All mathematical claims, experimental designs, and interpretations are the responsibility of the human authors.
\end{itemize}

\noindent We view AI-assisted formal verification as a methodology contribution in its own right. The type-checker guarantees that proofs are valid given the axioms; the human contribution is ensuring the axioms and theorem statements capture the intended mathematical claims. This division of labor --- human-conceived mathematics, AI-assisted proof construction, machine-verified correctness --- may be a template for future formalization efforts at scale.
...
\citet{chughtai2023toy} show that independently trained networks learn distinct internal representations for the same group operation, demonstrating empirical Rashomon at the circuit level. \citet{meloux2025circuits} find 85 distinct valid circuits with zero circuit error for XOR. Our component-level results extend this line of work: the instability in circuit importance is mathematically inevitable under architectural symmetry, and the $G$-invariant projection provides a principled resolution---averaging within orbits of the symmetry group to recover the stable shared structure.
...
\noindent\textbf{Lean formalization.} The 357-theorem, 6-axiom, 0-\texttt{sorry} formalization is available at \url{https://github.com/DrakeCaraker/dash-impossibility-lean} (archived at \url{https://doi.org/10.5281/zenodo.19468379}).
...
\textbf{AI assistance.} This work was developed with substantial assistance from Claude Code (Anthropic, Claude Opus 4.6). Specifically: (1) The Lean 4 formalization used AI-assisted proof construction --- all axioms and theorem statements were human-conceived; approximately 80\% of proof tactics were initially AI-drafted and then human-verified; all proofs were machine-checked by the Lean 4 type-checker with zero \texttt{sorry}. See Section~\ref{sec:ai-disclosure} for the detailed methodology. (2) Experiment scripts were co-developed with AI assistance; all experimental designs and interpretations are the responsibility of the human authors. (3) Paper text was written with AI drafting and editing assistance; all mathematical claims and scientific conclusions are the responsibility of the human authors. We view AI-assisted formal verification as a methodology contribution: the type-checker guarantees correctness given the axioms, while human judgment ensures the axioms and theorem statements capture the intended mathematical claims.
