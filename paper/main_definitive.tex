...
For binary attribution questions---SHAP sign, feature selection, counterfactual direction, circuit decomposition---the impossibility is strictly stronger: faithful + stable alone is impossible (the \emph{bilemma}). The bilemma has collapsed tightness, making it structurally more severe than the fairness impossibility of Chouldechova (2017): the only resolution is enrichment (adding neutral elements), not trade-off.

\textbf{Input-level.} On a 10{,}935-gene expression dataset, the \#1 biomarker alternates between TSPAN8 (80\% of seeds) and CEACAM5/CEA (20\%)---the drug target depends on the training seed. Under standard regularization, 45\% of German Credit applicants receive a different top explanation across equivalent models.

\textbf{Component-level.} Ten GPT-2-small models trained from scratch on OpenWebText produce 32 distinct top-5 component rankings with 8 different \#1 components across seeds. Within-layer head flip rate $= 0.500$ [0.482, 0.517]---a coin flip, as predicted. The $G$-invariant projection (orbit averaging over per-layer permutation symmetry) lifts agreement from $\rho = 0.277$ to $\rho = 0.873$ (Cohen's $d = 19.2$). On the IOI circuit, raw agreement is $\rho = 0.158$ (near chance), within-layer flip $= 0.499$, and 32 distinct top-5 rankings appear---component-level rankings are a lottery. At TinyStories scale (4L/4H and 6L/8H), the $G$-invariant projection achieves $\rho > 0.97$, and all four layer-0 heads appear as \#1 across 10 seeds, realizing the full $S_4$ symmetric group. SAE individual features are non-reproducible (overcompleteness guarantees Rashomon), but the dictionary's aggregate coverage geometry IS reproducible: projecting onto model activation PCs achieves $\rho = 0.960$ (vs.\ raw $-0.002$). The resolution maps 6{,}144 seed-dependent features to ${\sim}400$ seed-invariant directional densities.
...
The resolution at both levels is the \emph{stable projection} (orbit averaging): \DASH{} for features, $G$-invariant projection for circuits. \DASH{} is Pareto-optimal among unbiased aggregations, and the Reynolds operator provides the minimum-information-loss stable approximation at both levels. The achievable design space consists of exactly two families; no third option exists.

The framework is mechanically verified in Lean~4 (368 theorems from 6 axioms, 0~\texttt{sorry}; 139 with multi-step proofs of ${\geq}5$ tactic lines)---to our knowledge, the first formally verified impossibility in explainable AI. The formalization caught two logical inconsistencies and one type mismatch that survived informal review.
...
\noindent\textbf{The problem.} Importance rankings---of input features (SHAP) and internal components (activation patching)---are unreliable when interchangeable elements exist. Retraining the same model with a different random seed can invert which feature or component is ``most important.'' In a survey of 77 public datasets, 68\% exhibit input-level instability. Ten GPT-2-small models trained from scratch produce 32 distinct top-5 component rankings---component-level rankings are a lottery. Of the observed instability, approximately 63\% is attributable to model-level stochasticity and 37\% to evaluation-set variation (Section~\ref{sec:mechanism-isolation}).

\noindent\textbf{The theorem.} We prove this is not an engineering failure but a mathematical impossibility: no importance ranking can simultaneously be \emph{faithful} (reflect the model), \emph{stable} (robust to retraining), and \emph{complete} (rank all elements) when interchangeable elements exist under the Rashomon property. The proof requires only four lines and zero domain axioms. The impossibility operates at two levels: input-level (SHAP on collinear features) and component-level (activation patching on architecturally symmetric heads).
...
\begin{enumerate}
    ...
    \item \textbf{Resolve}: \DASH{} consensus averaging with $M \geq 25$ models reduces flip rate below 1\%. Uniquely optimal for binary $H$ (majority vote); optimal for continuous $H$ (Cauchy--Schwarz / Hunt--Stein).
...
\begin{itemize}
    \item Within-layer head flip rate $= 0.500$ [0.482, 0.517]---exactly a coin flip.
    ...
    \item SAE learned features are non-reproducible by overcompleteness (three conditions tested: TopK cosine $= 0.394$, ReLU cosine $= 0.136 \approx$ random, pretrained GPT-2 cosine $= 0.443$; resolution gives $+0.064$). The impossibility follows from the $8\times$ overcomplete null space, not from training methodology.
...
\noindent\textbf{Verification.} The framework is mechanically verified in Lean~4: 368 theorems from 6 axioms with 0 \texttt{sorry} across 59 files. The formalization caught three inconsistencies (two logical and one type mismatch) that survived informal review.
...
\begin{itemize}
  ...
  \item \textbf{Within-layer flip = coin flip}: $W_{\text{flip}} = 0.500$ [0.482, 0.517], exactly as predicted. Within each layer, ``which head matters most?'' is determined by the random seed, not by the data.
...
\begin{itemize}
  ...
  \item \textbf{Within-layer flip = coin flip}: $W_{\text{flip}} = 0.499$, confirming the impossibility at the task-specific level.
...
\paragraph{PC resolution: dictionary coverage geometry.}
We test whether projecting SAE feature properties onto the principal components of model activations produces reproducible rankings. The PCs are computed from 400 random-token sequences passed through the base model's layer~6 (SAE-seed-invariant, as all SAEs share the same frozen model). For each SAE seed, we compute the \emph{per-direction coverage}: $w_j = \sum_i \text{imp}_i \cdot (d_i \cdot u_j)^2 / \|d_i\|^2$, measuring how much total decoder mass each seed allocates to each PC direction.

\begin{table}[ht]
\centering
\caption{PC resolution for SAE features. Projecting decoder coverage onto model activation PCs produces $\rho = 0.960$, demonstrating that the dictionary's aggregate geometry is seed-invariant. Note: the $G$-invariant comparison (attention heads, $\rho = 0.873$) involves a harder task---10 different models vs.\ 10 SAEs on the same model---and is shown for context, not as a direct comparison.}
\label{tab:pc-resolution}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{Spearman $\rho$} & \textbf{Dimensions} \\
\midrule
Raw SAE feature ranking & $-0.002$ & 6{,}144 \\
Hungarian matching (prior) & 0.500 & 500 \\
$G$-invariant (attention heads, different task$^*$) & 0.873 & 24 \\
\textbf{PC projection (SAE coverage)} & \textbf{0.960} & \textbf{400} \\
\midrule
\multicolumn{3}{l}{\emph{By PC dimensionality (limited by $n_{\text{samples}} = 400$):}} \\
\quad Top-10 PCs & 0.888 & 10 \\
\quad Top-50 PCs & 0.811 & 50 \\
\quad Top-100 PCs & 0.888 & 100 \\
\quad Top-200 PCs & 0.937 & 200 \\
\quad Top-400 PCs (all available) & 0.960 & 400 \\
\bottomrule
\multicolumn{3}{l}{\footnotesize $^*$10 different models trained from scratch vs.\ 10 SAEs on one frozen model.}
\end{tabular}
\end{table}

\paragraph{What is being measured.}
TopK SAEs normalize decoder columns to unit norm during training. With $\|d_i\| \approx 1$ for all features, the ``importance'' (L2 norm) is approximately constant. The PC projection therefore measures the \emph{density of decoder directions} in each PC direction---i.e., how many features point toward each principal axis of the data. This is the dictionary's \textbf{coverage geometry}: how it allocates representational capacity across data directions. The coverage geometry is determined by the reconstruction objective (which forces the dictionary to cover all high-variance directions), not by the SAE random seed.

\paragraph{Interpretation and scope.}
The $\rho = 0.960$ result demonstrates that SAE dictionaries trained with different seeds converge to the same aggregate coverage geometry, even though individual feature-to-direction assignments differ. This is analogous to: ``every SAE covers the same landscape, just with different hiking trails.'' The trails (individual features) are non-reproducible; the landscape (coverage per direction) is.

\textbf{What this does NOT yet show:} whether \emph{activation-weighted importance} (the standard behavioral metric: how often and how strongly each feature fires on real data) is also reproducible under PC projection. The L2-norm-based coverage geometry is reproducible because the reconstruction objective forces it. Activation-weighted importance depends on the SAE encoder (not just decoder) and may have additional non-reproducible structure. This is the critical open experiment.

\paragraph{Why model activation PCs work but importance PCA fails.}
...
\begin{itemize}
  ...
  \item \textbf{PCA on model activations} (across inputs): finds directions of maximum \emph{variation in the data} $=$ the structure every SAE must cover. Result: $\rho = 0.960$.
\end{itemize}
The first identifies what changes; the second identifies what's shared. This distinction is the empirical version of the theoretical claim: the reproducible subspace is orthogonal to the symmetry noise.

\begin{itemize}
  \item \textbf{Heads}: average importance within each layer $\to$ $L$-dimensional invariant ($L = 12$). Group: $S_{12}$ per layer (known, finite). Task: 10 different models.
  \item \textbf{SAE features}: project onto activation PCs $\to$ ${\sim}200$--$400$-dimensional invariant$^\dagger$. Group: null-space rotations (continuous). Task: 10 SAEs on one model.
\end{itemize}
{\footnotesize $^\dagger$Limited by $n_{\text{samples}} = 400$; true effective dimension may be higher with more PCA samples.}

Both sacrifice feature-level granularity for stability---the Design Space Theorem's faithfulness-completeness tradeoff. The PC resolution provides a candidate Reynolds operator for the dictionary's continuous symmetry group, though formal proof that it IS the Reynolds operator (rather than a fortuitously effective projection) remains open.
