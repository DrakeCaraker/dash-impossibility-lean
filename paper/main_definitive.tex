...
This establishes that \DASH{} and the $G$-invariant projection $V^G$ minimize information loss among \emph{all} stable (i.e., $G$-invariant) approximations --- not just unbiased or linear ones. The result is machine-verified: \texttt{reynolds\_best\_approximation} in the universal impossibility formalization proves the inequality for any idempotent self-adjoint linear map on an inner product space.
