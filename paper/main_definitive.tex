...
For any two functionally equivalent neural networks with incompatible circuit decompositions, no explanation method $E$ can be simultaneously faithful and stable. The Rashomon property is \textbf{derived} from constructive finite types (zero axioms).
\end{theorem}
...
The mechanistic interpretability impossibility, bilemma, and unfaithfulness bound are machine-verified in \texttt{MechInterp.lean} with zero axiom dependencies. All types are finite inductives; the Rashomon property is derived via \texttt{decide}, not axiomatized.
...
\paragraph{Lean formalization.}
The bilemma, unfaithfulness bound, all-or-nothing theorem, and both ML instances (SHAP sign, feature selection) are machine-verified in \texttt{Bilemma.lean} with zero axiom dependencies beyond the Rashomon property.
