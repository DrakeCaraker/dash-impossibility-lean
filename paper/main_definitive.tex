...
\paragraph{The correct resolution for SAEs: PC projection.}
The capacity theorem implies that the natural basis for the reproducible subspace $V^G$ is \textbf{not} the SAE's feature directions (which are seed-dependent) but the data's principal directions (which are seed-invariant). The correct resolution: project feature importance onto the data PCs and report importance in that basis. This is the SAE analogue of per-layer averaging for attention heads:
\begin{itemize}
  \item \textbf{Heads}: average importance within each layer $\to$ $L$-dimensional invariant ($L = 12$ for GPT-2)
  \item \textbf{SAE features}: project onto data PCs $\to$ $d$-dimensional invariant ($d = 768$)
\end{itemize}
