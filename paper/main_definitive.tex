...
We quantify the impossibility for four model classes, resolve it via SHAP (SHapley Additive exPlanations) ensemble averaging with \DASH{}, and machine-verify it with 335 Lean~4 theorems.
...
\noindent\textbf{Verification.} The framework is mechanically verified in Lean~4: 335 theorems from 16 axioms with 0 \texttt{sorry} across 58 files. The formalization caught two logical inconsistencies that survived informal review.
...
This document is the complete reference: impossibility theorem, quantitative bounds, constructive resolution, three Symmetric Bayes Dichotomy (SBD) instances, diagnostics, experiments across 11 datasets and 3 GBDT implementations, financial case studies, and a 335-theorem Lean~4 formalization.
...
\begin{sloppypar}
The proof is mechanically verified in Lean~4 using Mathlib: 335 type-checked theorems
and lemmas across 58 files (110 with multi-step proofs of ${\geq}5$ tactic lines; the
...
\begin{sloppypar}
The formalization comprises 58 Lean~4 files with 16 axioms and 335 type-checked
theorems and lemmas (0 \texttt{sorry}). Of these, 110 require multi-line proofs
...
\begin{table}[t]
\centering
\caption{Lean file cross-reference (58 files, 335 theorems, 16 axioms).}
