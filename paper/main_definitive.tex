...
\title{The Attribution Impossibility: Faithful, Stable, and Complete Feature Rankings Cannot Coexist Under Collinearity\\[8pt]\large Definitive Reference (Complete Version)}
...
When reporting SHAP rankings for a model with collinear features: ``Features [X, Y, Z] form a correlated group ($|\rho| > [\text{threshold}]$). Their relative ranking is unstable across training seeds (estimated flip rate: [X]\%). They should be interpreted as interchangeable contributors. The between-group ranking is stable ($Z > 1.96$).'' For regulatory reporting (EU AI Act, SR~11-7), the \DASH{} consensus ranking provides a defensible explanation.
...
For stumps, the axiom holds reasonably well; for deeper trees, multi-level interactions increase the CV. The proportionality axiom is used only for the quantitative bounds (Theorems~\ref{thm:ratio}); the core impossibility (Theorem~\ref{thm:impossibility}) does not depend on it.
...
To validate on the exact collinearity pattern common in US credit models, we generate 10{,}000 samples with 5 features driven by a latent ``creditworthiness'' factor: income ($\rho = 0.99$ with DTI), DTI, credit score ($\rho = 0.98$ with income), loan amount (independent), and employment years ($\rho = 0.94$ with income). With 25 XGBoost models: income$\leftrightarrow$DTI flips 8\% ($|\rho| = 0.99$); credit score$\leftrightarrow$employment years flips 12\% ($|\rho| = 0.92$). Pairs where one feature clearly dominates (income vs.\ credit score) show 0\% flips despite $|\rho| = 0.98$---confirming the impossibility requires BOTH high correlation AND similar importance. For SR~11-7 compliance: report income and DTI as ``interchangeable key risk drivers'' rather than forcing a ranking that would flip under retraining.
...
We load DistilBERT and create 10 model variants by adding small random perturbations ($\sigma = 0.02$) to attention weights, simulating different training runs. Of 59 adjacent token pairs across test sentences, 88\% have flip rates exceeding 10\%. Examples: ``extremely''/``poor'' flips 50\% (coin flip), ``not''/``bad'' flips 40\%. Mean Spearman correlation of full-sentence token rankings is 0.637.

Under actual fine-tuning on SST-2 (5 seeds, 1 epoch, AdamW, lr$=2{\times}10^{-5}$), 14.5\% of pairs are unstable. The identical 14.5\% across 1 and 3 epochs indicates the instability is structural (driven by collinearity in attention patterns), not a training-depth artifact.
...
The Lean formalization contains two Spearman bounds: (i) \texttt{spearmanCorr\_bound}, fully derived from the definition of Spearman correlation, giving $\rho_S \leq 1 - 3/(P^3 - P)$; and (ii) \texttt{spearman\_classical\_bound}, axiomatized, giving the tighter $\rho_S \leq 1 - m^3/P^3$. The paper uses the tighter axiomatized bound. The full combinatorial proof (bounding the expected $\sum d_i^2$ under random tie-breaking) requires counting arguments not yet available in the Lean framework.
...
The Attribution Impossibility and Arrow's theorem are \emph{analogous} but not instances of a common theorem in any non-trivial sense. A common framework can be defined, but its ``impossibility theorem'' reduces to: ``if instances disagree and the aggregation rule must be constant, faithfulness is impossible''---a one-line observation. Arrow's actual content (the impossibility persists even when the rule is allowed to depend on all voters) has no analogue in attribution. The closest structural analogue may be Sen's (1970) impossibility of a Paretian liberal, where individual ``rights'' conflict with the Pareto criterion.
...
For $m$ features, the attribution vector lives in $\R^m$. The ranking is determined by the chamber of the braid arrangement $\{x_i = x_j : i \neq j\}$. One might hope the attribution map has a non-trivial winding number around the intersection of hyperplanes, giving a quantitative lower bound on ranking flips. This does not work: (1) the complement of the braid arrangement in $\R^m$ is simply connected for $m \geq 3$; (2) even if a winding number existed, it would count signed crossings, and the flip rate (unsigned) is already captured by $1/(1-\rho^2)$.

\subsection{Assessment}

The Attribution Impossibility is fundamentally a \emph{combinatorial} result (conflicting preferences cannot all be respected), not a topological one. It holds for arbitrary model spaces (including finite, discrete spaces with no topology). Attempts to extract topological content either reduce to the trivial connectedness argument or require unjustified smoothness assumptions.

\section{On Categorical/Axiomatic Strengthening}

\subsection{The Trivial Version}

Let $A_1, \ldots, A_n$ be any finite set of axioms. If $A_1 \wedge \cdots \wedge A_n$ implies faithfulness, stability, and completeness, then $A_1 \wedge \cdots \wedge A_n \to \bot$ under the Rashomon property. This is immediate from Theorem~\ref{thm:impossibility}; no content is added.
...
The ``categorical impossibility'' does not genuinely extend Theorem~\ref{thm:impossibility}. The quantitative version provides useful bounds but follows directly from DGP symmetry.
