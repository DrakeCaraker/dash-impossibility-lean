...
The design space theorem, diagnostics, and impossibility are mechanically verified in Lean~4 (340 theorems from 14 domain-specific + 2 query-complexity axioms, 0 \texttt{sorry}; 115 with multi-step proofs of ${\geq}5$ tactic lines; the remainder are definitions, wrappers, and single-step applications)---to our knowledge, the first formally verified impossibility in explainable AI.
...
\noindent\textbf{Verification.} The framework is mechanically verified in Lean~4: 340 theorems from 16 axioms with 0 \texttt{sorry} across 58 files. The formalization caught three inconsistencies (two logical and one type mismatch) that survived informal review.
...
\begin{sloppypar}
The proof is mechanically verified in Lean~4 using Mathlib: 340 type-checked theorems
...
\begin{enumerate}
    ...
    \item \textbf{Machine-verified proof}: formalized in Lean~4 (340 type-checked theorems from 16 axioms, 0 \texttt{sorry}), publicly available at \url{https://github.com/DrakeCaraker/dash-impossibility-lean} (DOI: \url{https://doi.org/10.5281/zenodo.19468379}). The axiom system is consistent: we construct an explicit model satisfying all 14 domain-specific axioms simultaneously, both in Lean (\texttt{Consistency.lean}) and numerically.
\end{enumerate}
\end{sloppypar}
...
The formalization uses 16 axioms total (14 domain-specific axioms in \texttt{Defs.lean} plus 2 query-complexity scaling constants in \texttt{QueryComplexity.lean}): 6 type/constant declarations, 2 measure-theoretic infrastructure axioms, 6 domain-specific property axioms, and 2 query-complexity axioms from Le~Cam's method.
...
\begin{sloppypar}
The formalization comprises 58 Lean~4 files with 16 axioms and 340 type-checked
...
Lean verification guarantees that all theorems follow from the stated axioms with no logical gaps (internal consistency). It does not guarantee that the axioms correctly model the phenomena they are intended to capture (external validity). The 16 axioms are motivated by the structure of gradient-boosted decision trees and SHAP value computation (\S\ref{sec:axiom-audit}), but their adequacy for any particular model or dataset is an empirical question.
...
\begin{table}[t]
\centering
\caption{Lean file cross-reference (58 files, 340 theorems, 16 axioms).}
