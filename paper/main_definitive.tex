...
\title{The Attribution Impossibility:\\{\large No Importance Ranking Is Faithful, Stable, and Complete Under Symmetry}\\[8pt]\normalsize Definitive Reference (Complete Version)}
...
\begin{abstract}
On Breast Cancer Wisconsin, 50 retrains produce 24 distinct top-3 SHAP rankings. Ten GPT-2-small models trained from scratch produce 32 distinct top-5 component rankings with 8 different \#1 attention heads. When correlated features have similar importance, the ranking is determined by the training seed.

The root cause is a conflict among three desiderata: no importance ranking can be simultaneously faithful (agree with the model's attributions), stable (consistent across equivalent models), and complete (order all elements) when the Rashomon property holds---when equivalent models rank elements in opposite orders. We characterize the \textbf{complete design space}: exactly two families of methods exist, no third option is possible, and \DASH{} (ensemble orbit averaging) is the optimal method on the stable branch. \DASH{} is the orbit average under the natural symmetry group; by classical invariant decision theory (Hunt--Stein, Rao--Blackwell), it is the optimal equivariant estimator, achieving Cauchy--Schwarz-optimal variance. For binary attribution questions (SHAP sign, feature selection), faithful + stable alone is impossible---a corollary with collapsed tightness that forecloses approximate fixes.

The instability is quantitative. For gradient-boosted stumps, the attribution ratio diverges as $1/(1{-}\rho^2)$; this bound is stump-specific, though the qualitative pattern---GBDT diverges, Lasso is infinite, random forests converge---holds across depths. A nonparametric diagnostic (the minority fraction) predicts flip rates in seven lines of code (Spearman $0.96$ vs.\ $0.46$ for the Gaussian formula on California Housing). In a survey of 77 datasets with $M{=}20$ models, at least 52 (68\%, 95\% CI: [56\%, 77\%]) show flip rates exceeding 10\%, though detection power at this threshold is approximately 32\%.

\textbf{Component-level.} Ten GPT-2-small models trained from scratch produce within-layer head flip rate $= 0.500$ [0.482, 0.517]---near 50\% as predicted for symmetric components. The $G$-invariant projection lifts agreement from $\rho = 0.277$ to $0.873$ (Cohen's $d = 19.2$). At TinyStories scale, $\rho_{G\text{-inv}} > 0.97$ with all four layer-0 heads appearing as \#1 (full $S_4$). Two different ablation methods (weight zeroing, mean ablation) disagree on raw scores ($\rho \approx 0.5$) but converge after orbit averaging ($\rho > 0.97$)---the strongest direct evidence for the symmetry framework. SAE individual features are non-reproducible (overcomplete null space); coarse-grained behavioral allocation is partially reproducible ($\rho = 0.878$ at top-10 PCs, degrading to $0.711$ at fine grain).

The framework is mechanically verified in Lean~4 (368 theorems, 6 axioms, 0~\texttt{sorry}). The formalization caught two logical inconsistencies and one type mismatch that survived informal review.

Code and Lean proofs: \url{https://github.com/DrakeCaraker/dash-impossibility-lean} (DOI: \url{https://doi.org/10.5281/zenodo.19468379}).
\end{abstract}

\section*{Executive Summary}

\noindent\textbf{The problem.} Importance rankings---of input features (SHAP) and internal components (activation patching)---are unreliable when correlated elements have similar importance. Retraining the same model with a different random seed can change which feature or component ranks highest. In a survey of 77 public datasets ($M{=}20$ models each), at least 68\% show flip rates exceeding 10\% (detection power ${\sim}32\%$ at this threshold; true prevalence may be higher). Ten GPT-2-small models produce 32 distinct top-5 component rankings. Of the observed instability, approximately 63\% is attributable to model-level stochasticity and 37\% to evaluation-set variation (Section~\ref{sec:mechanism-isolation}).

\noindent\textbf{The root cause.} The three desiderata---faithfulness (agree with the model), stability (consistent across retrains), completeness (rank all elements)---cannot be jointly satisfied when the Rashomon property holds. This observation is immediate once the definitions are stated: if equivalent models rank elements in opposite orders, no single ranking can agree with all of them. The contribution is not in this observation itself, but in identifying the right abstraction and fully characterizing its consequences.

\noindent\textbf{The design space.} The complete set of achievable methods consists of exactly two families:
\begin{itemize}
    \item \textbf{Family A} (single-model): Faithful and complete, but rankings flip---up to 50\% for maximally symmetric pairs, typically 15--30\% at moderate collinearity.
    \item \textbf{Family B} (\DASH{} ensemble): Stable, but reports ties for symmetric features. By classical invariant decision theory (Hunt--Stein), the orbit average is the optimal equivariant estimator under the natural symmetry group.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Screen}: The single-model screen (94\% precision; tree-based models) identifies unstable pairs from one model.
    \item \textbf{Predict}: The minority fraction---a 7-line nonparametric diagnostic derived from coverage conflict theory---predicts flip rates with Spearman $0.92$--$0.98$, outperforming the Gaussian formula ($0.46$--$0.89$) by $2\times$ on real data. Model-class universal (XGBoost, RF, Ridge, LASSO).
    \item \textbf{Validate}: The multi-model Z-test confirms instability with 5 models.
    \item \textbf{Resolve}: \DASH{} consensus averaging with $M \geq 25$ models reduces flip rate below 1\%. As the orbit average under the natural symmetry group, DASH is the optimal equivariant estimator by classical invariant decision theory.
    \item \textbf{Size}: The ensemble formula $M_{\min} = \lceil 2.71 \cdot \sigma^2/\Delta^2 \rceil$ gives the optimal model count.
\end{enumerate}

\noindent\textbf{Component-level results.}
Ten GPT-2-small models trained from scratch on OpenWebText (3B tokens each, different random seeds) show:
\begin{itemize}
    \item Within-layer head flip rate $= 0.500$ [0.482, 0.517]---near 50\%, as predicted for symmetric components.
    \item Raw agreement $\rho = 0.277$; after $G$-invariant projection, $\rho = 0.873$ (Cohen's $d = 19.2$).
    \item 32 distinct top-5 component rankings, 8 different \#1 components across 10 seeds.
    \item On the IOI (Indirect Object Identification) circuit: raw $\rho = 0.158$, within-layer flip $= 0.499$, 32 distinct top-5 rankings.
    \item SAE individual features are non-reproducible (overcomplete null space). Coarse behavioral allocation is partially reproducible ($\rho = 0.878$ at top-10 PCs, $0.711$ at all 768).
    \item 7/7 theory-derived predictions PASS.
\end{itemize}
...
\noindent\textbf{Regulatory implications.} If the Rashomon conditions hold for a deployed model, single-model SHAP audits for proxy discrimination are mathematically unreliable---the audit conclusion changes across training seeds. A regulator might reasonably treat undisclosed instability as a foreseeable risk under frameworks such as EU AI Act Art.~13(3)(b)(ii), though this legal assessment requires domain expertise beyond the scope of this paper.

\noindent\textbf{Verification.} The framework is mechanically verified in Lean~4 (368 theorems, 6 axioms, 0 \texttt{sorry}, 59 files; 139 with multi-step proofs). The formalization served as a bug-finding methodology, catching two logical inconsistencies and one type mismatch that survived informal review.
...
is unstable---the same model architecture, same data, different
random seed produces a different answer. Under the Equal Credit Opportunity Act (ECOA), the lender's adverse action notice
...
\begin{sloppypar}
The proof is mechanically verified in Lean~4 using Mathlib: 368 type-checked theorems
and lemmas across 59 files (139 with multi-step proofs of ${\geq}5$ tactic lines; the
...
\label{def:non-degeneracy}
Features $j, k$ are \emph{non-degenerate} if there exists at least one model $f$ with $\varphi_j(f) \neq \varphi_k(f)$.
\end{definition}

\begin{remark}
Non-degeneracy is an existential condition: it requires only \emph{one} model that distinguishes the features. This is trivially satisfied in practice---any trained model on collinear features produces unequal attributions. Unlike a probability-1 claim (which would require a transversality argument), the existential requires no measure-theoretic machinery.
\end{remark}

\begin{definition}[Algorithmic symmetry]
\label{def:algo-sym}
A model class satisfies \emph{algorithmic symmetry} for features $j, k$ if: for any model $f$ with $\varphi_j(f) > \varphi_k(f)$, there exists a model $f'$ with $\varphi_k(f') > \varphi_j(f')$ (and vice versa).
\end{definition}

\begin{remark}
Algorithmic symmetry follows directly from Theorem~\ref{thm:rashomon-symmetry}: if $\varphi_j(f) > \varphi_k(f)$ and the model class is permutation-closed under a symmetric DGP, then $f' = f \circ \pi_{jk}$ satisfies $\varphi_k(f') > \varphi_j(f')$ with the same population loss. No probabilistic argument is needed.
\end{remark}

\begin{theorem}[Rashomon inevitability]
\label{thm:rashomon-inevitability}
Let $\mathcal{F}$ be permutation-closed under a symmetric DGP. If features $j, k$ are non-degenerate (Definition~\ref{def:non-degeneracy}), then the Rashomon property holds for the pair $(j, k)$: there exist models $f, f' \in \mathcal{F}$ with $\varphi_j(f) > \varphi_k(f)$ and $\varphi_k(f') > \varphi_j(f')$.
\end{theorem}

\begin{proof}
By non-degeneracy, there exists $f$ with $\varphi_j(f) \neq \varphi_k(f)$. Without loss of generality, $\varphi_j(f) > \varphi_k(f)$. By algorithmic symmetry (which follows from Theorem~\ref{thm:rashomon-symmetry}: take $f' = f \circ \pi_{jk}$), there exists $f'$ with $\varphi_k(f') > \varphi_j(f')$. Both directions are realized.
\end{proof}

\begin{remark}
This proof is fully machine-verified in Lean (\texttt{rashomon\_inevitability} in \texttt{RashomonInevitability.lean}) with zero axiom dependencies beyond the type declarations. The only hypotheses are non-degeneracy (an existential) and algorithmic symmetry (a consequence of permutation closure). No measure theory, no transversality argument, no probability-1 claim.
\end{remark}

\begin{remark}
Theorem~\ref{thm:rashomon-inevitability} makes the Attribution Impossibility inescapable for any standard ML pipeline under collinearity. The only escapes are: (i)~perfect degeneracy ($\varphi_j(f) = \varphi_k(f)$ for ALL models---impossible in practice), (ii)~a model class that is NOT permutation-closed (hard-coded feature preferences), or (iii)~a DGP that is NOT symmetric within the group.
\end{remark}
...
\subsection{DASH Information Loss (Explanatory Information Loss)}
\label{sec:info-loss}

\DASH{} achieves stability by discarding model-specific within-group information---the \emph{explanatory information loss} of the stable projection. For a group of $m$ symmetric features, a single model produces a complete ranking (one of $m!$ possible orderings). Since each of the $m!$ orderings is equally likely under DGP symmetry, the ranking carries $\log_2(m!)$ bits of information about the specific model. \DASH{} reports a tie, discarding the entire $\log_2(m!)$ bits. In capacity terms: the \emph{explanation capacity} for input-level attribution with $L$ groups of $m$ features each is $C = L$ (one group-level importance per group), and the \emph{explanation loss rate} is $\eta = 1 - L/(Lm) = 1 - 1/m$. For typical group sizes:
...
When features correlated with a protected attribute have similar importance to non-protected features, two auditors examining the \emph{same model architecture trained on the same data} with different random seeds may reach opposite conclusions about proxy reliance.

If the Rashomon conditions hold for a deployed model, a regulator might reasonably treat undisclosed instability as a foreseeable risk under frameworks such as EU AI Act Art.~13(3)(b)(ii); however, this legal assessment requires domain expertise beyond the scope of this paper. The mathematical remedy is ensemble auditing: train $M$ models and use \DASH{} consensus.
...
The impossibility results of \citet{chouldechova2017fair} (calibration $P_1$ vs.\ equal FPR $P_2$ vs.\ equal FNR $P_3$) and \citet{kleinberg2017inherent} (same conclusion, different proof) can be cast as instances of the abstract impossibility framework: each defines an explanation system where the ``explanation'' is a fairness metric, the Rashomon property arises from base-rate differences, and the three fairness desiderata play the roles of faithful, stable, and decisive.

\paragraph{Tightness transition under performance constraints.}
We apply the framework's tightness classification to the fairness trilemma. The result is a \emph{transition}: the tightness type changes under performance constraints.

\begin{table}[ht]
\centering
\caption{Fairness impossibility tightness transition under TPR constraints. At practical performance levels, calibration is the universal bottleneck.}
\label{tab:fairness-tightness}
\small
\begin{tabular}{@{}llp{7cm}@{}}
\toprule
\textbf{TPR constraint} & \textbf{Tightness type} & \textbf{Achievable pairwise compromises} \\
\midrule
...
At $\tau = 0$ (no performance constraint), the tightness is \textbf{full}: all three pairwise compromises are achievable. However, the classifiers achieving them are degenerate --- they classify (almost) nobody as positive, achieving perfect calibration and equal error rates trivially. This is technically correct but practically vacuous.

At $\tau \geq 0.3$ (TPR $\geq$ 30\% for both groups --- a minimal practical requirement), the tightness transitions to \textbf{PPV-blocked}: calibration (PPV parity, $P_1$) is individually incompatible with both equal FPR ($P_1{+}P_2$ gap $> 0.10$) and equal FNR ($P_1{+}P_3$ gap $> 0.13$). The only achievable pairwise fairness compromise is equalized odds ($P_2{+}P_3$, gap $< 0.006$).

\paragraph{Validation.}
Tested on Adult Income (sex as protected attribute, base rate gap 0.20) and two synthetic datasets (base rate gaps 0.25 and 0.40). Methodology: proper train/test split (70/30 stratified), group-specific thresholds ($100 \times 100 = 10{,}000$ classifiers per model), 3 base model classes (logistic regression, random forest, gradient boosting), and bootstrap with model retraining (20 iterations, 100\% stable classification). For the synthetic dataset with base rate gap 0.40, the tightness is PPV-blocked even at $\tau = 0$ --- larger base rate differences make the impossibility bind earlier.

\paragraph{Practical implication.}
The tightness transition changes the recommendation from ``choose a fairness criterion and accept the trade-off'' (the standard framing) to ``equalized odds is the only achievable criterion at practical performance; calibration is structurally incompatible with either component of equalized odds.'' This is a structural finding: it holds regardless of the model class, the threshold choice, or the optimization method. The framework's value is diagnostic --- it identifies not just \emph{that} an impossibility exists, but \emph{which} compromises are available and \emph{when} the impossibility becomes binding.

\paragraph{Reynolds best-approximation optimality.}
The orbit average (Reynolds projection) $Rv = \frac{1}{|G|}\sum_{g \in G} g \cdot v$ satisfies a \emph{best-approximation theorem}: for any $G$-invariant vector $w$,
\[
\|v - Rv\| \leq \|v - w\|.
\]
...
This establishes that \DASH{} and the $G$-invariant projection $V^G$ minimize information loss among \emph{all} stable (i.e., $G$-invariant) approximations --- not just unbiased or linear ones. The \emph{explanation capacity} $C = \dim(V^G)$ measures how much attribution information survives the projection: for $G = S_k^L$, $C = 2L$ (one head-average and one MLP per layer); the \emph{explanation loss rate} $\eta = 1 - C/\dim(V)$ is the fraction of the attribution vector lost to within-orbit variation. The \emph{Explanation Capacity Theorem}: the expected fraction of explanation content preserved by orbit averaging is exactly $C/\dim(V) = 1 - \eta$, determined by the symmetry group $G$ alone. \DASH{} and $V^G$ are the \emph{stable projections}---the optimal encodings that achieve the capacity bound with equality.

The result is machine-verified: \texttt{reynolds\_best\_approximation} in the universal impossibility formalization proves the inequality for any idempotent self-adjoint linear map on an inner product space \citep{caraker2026limits}.
...
Circuit importance via activation patching IS attribution---attributing importance to internal components rather than input features. The Attribution Impossibility (Theorem~\ref{thm:impossibility}) applies directly: activation patching scores are attributions ($\varphi_j$), independently trained networks are configurations ($\Theta$), and architectural symmetry ($S_k^L$, the symmetric group permuting $k$ attention heads within each of $L$ layers) provides the interchangeability that generates the Rashomon property. The same theorem, operating at a different granularity, predicts the same instability and admits the same resolution: orbit averaging via the $G$-invariant projection.

\paragraph{The $G$-invariant projection as Reynolds operator.}
The resolution for component-level attribution is the \emph{stable projection}---the Reynolds operator (orbit average)---over the symmetry group $G = \prod_{\ell=1}^{L} S_{k_\ell}$, where $S_{k_\ell}$ permutes the $k_\ell$ attention heads in layer $\ell$. For an importance vector $v \in \mathbb{R}^n$, the $G$-invariant projection is $v^G_\ell = \frac{1}{k_\ell} \sum_{h=1}^{k_\ell} v_{\ell,h}$---the mean importance of heads within each layer. This is precisely the Reynolds operator $\mathcal{R}_G(v) = \frac{1}{|G|} \sum_{g \in G} g \cdot v$, which projects onto the subspace of $G$-invariant vectors. Components outside the symmetry group (MLP layers, layer norms) are invariant singletons and pass through unchanged. The $G$-invariant projection reduces the $n$-dimensional attribution problem to an $n_{\text{inv}}$-dimensional one: for Config~A ($n=20$, 4 heads/layer $\times$ 4 layers + 4 MLPs), $n_{\text{inv}} = 8$ (4 head-averages + 4 MLPs); for Config~B ($n=54$, 8 heads/layer $\times$ 6 layers + 6 MLPs), $n_{\text{inv}} = 12$.
...
\subsection{TinyStories Multi-Scale Circuit Stability}
\label{sec:tinystories}

\paragraph{Setup.}
We train 10 transformers from independent random seeds on TinyStories~\citep{eldan2023tinystories} at three scales:
\begin{itemize}
  \item \textbf{Config~A} (Small): 4 layers, 4 heads, $d_{\text{model}} = 256$, $n = 20$ components (16 heads + 4 MLPs).
  \item \textbf{Config~B} (Medium): 6 layers, 8 heads, $d_{\text{model}} = 512$, $n = 54$ components (48 heads + 6 MLPs).
  \item \textbf{Config~C} (GPT-2 fine-tuned): 12 layers, 12 heads, $d_{\text{model}} = 768$, $n = 156$ components (144 heads + 12 MLPs). This serves as a boundary condition: fine-tuning from a shared checkpoint does not create genuine Rashomon diversity.
\end{itemize}
All 10 models within each configuration achieve near-identical perplexity (Config~A: mean PPL $= 8.584$, CV $= 0.659\%$; Config~B: mean PPL $= 9.976$, CV $= 1.033\%$; Config~C: mean PPL $= 18.539$, CV $= 0.076\%$).
Component importance is measured by activation patching (weight zeroing): zero out each component's parameters and measure the change in loss.
Seven theory-derived predictions are evaluated per configuration; all 7/7 pass for Configs~A and~B, while Config~C passes 4/7 (correctly predicted by the theory as a non-Rashomon regime).

\paragraph{Results.}
Table~\ref{tab:tinystories-results} presents the full results across all three scales.

\begin{table}[ht]
\centering
\caption{TinyStories component-level attribution stability across three architectural scales. $\rho_{\text{full}}$: mean pairwise Spearman correlation of raw importance vectors. $\rho_{G\text{-inv}}$: Spearman correlation after $G$-invariant projection. $W_{\text{flip}}$: within-layer head flip rate. $B_{\text{flip}}$: between-group (head vs.\ MLP) flip rate. $d$: Cohen's $d$ for within vs.\ between flip rate separation. Split-half: within-model Spearman reliability. RP pctile: percentile of $G$-invariant $\rho$ under random projection null. Perm.\ $p$: permutation test $p$-value.}
...
$n_{\text{inv}}$ (after $G$-proj.) & 8 & 12 & 24 \\
PPL mean (CV\%) & 8.584 (0.659\%) & 9.976 (1.033\%) & 18.539 (0.076\%) \\
\midrule
$\rho_{\text{full}}$ [95\% CI] & 0.565 [0.527, 0.603] & 0.540 [0.516, 0.565] & 0.993 [0.993, 0.994] \\
$\rho_{G\text{-inv}}$ [95\% CI] & 0.972 [0.966, 0.979] & 0.982 [0.977, 0.986] & 0.999 [0.999, 1.000] \\
\midrule
$W_{\text{flip}}$ [95\% CI] & 0.496 [0.467, 0.526] & 0.489 [0.478, 0.500] & 0.043 [0.041, 0.045] \\
$B_{\text{flip}}$ & 0.000 & 0.000 & 0.000 \\
...
\paragraph{Per-seed head importance: full $S_4$ realization.}
Table~\ref{tab:tinystories-seeds} shows which attention head ranks highest in each seed for Config~A (excluding MLP components, which are invariant singletons and always dominate). All four layer-0 heads appear as the top-ranked head across the 10 seeds, realizing the full symmetric group $S_4$.

\begin{table}[ht]
\centering
\caption{Top-ranked attention head per seed in Config~A (4 layers $\times$ 4 heads). All four layer-0 heads appear as \#1, realizing the full $S_4$ permutation symmetry. MLP components (which dominate overall) are excluded---they are invariant singletons outside the symmetry group.}
\label{tab:tinystories-seeds}
\small
\begin{tabular}{cclr}
\toprule
\textbf{Seed} & \textbf{Component idx} & \textbf{Head} & \textbf{Importance} \\
\midrule
0 & 2 & L0H2 & 1.153 \\
1 & 3 & L0H3 & 1.597 \\
2 & 2 & L0H2 & 0.540 \\
...
This is the component-level analogue of the input-level impossibility: ``which head matters most?'' depends entirely on which random seed was used, even though all models achieve the same perplexity. The $G$-invariant projection resolves this by reporting the \emph{layer-averaged} importance, which is stable across all seeds.

\paragraph{Controls.}
Three controls validate that the $G$-invariant lift is not an artifact:
\begin{enumerate}
  \item \textbf{Split-half reliability}: within-model measurements are highly precise (Spearman $= 0.991$ for Config~A, $0.960$ for Config~B), far exceeding between-model agreement ($0.565$, $0.540$). The instability is between models, not within.
  \item \textbf{Random projection null}: the $G$-invariant projection achieves the 100th percentile among 1{,}000 random projections to the same dimensionality (heads-only) at both scales. No random $n_{\text{inv}}$-dimensional subspace achieves comparable stability.
  \item \textbf{Permutation test}: $p < 0.001$ at both scales (Config~A: actual $\rho_{G\text{-inv}} = 0.972$, null mean $= 0.836$, null SD $= 0.016$; Config~B: actual $= 0.982$, null mean $= 0.864$, null SD $= 0.010$).
\end{enumerate}

\paragraph{GPT-2 boundary condition.}
Config~C (GPT-2 fine-tuned from a shared checkpoint) provides a crucial negative control. Raw agreement is already near-perfect ($\rho_{\text{full}} = 0.993$), and the $G$-invariant lift is negligible ($\rho_{G\text{-inv}} = 0.999$). The within-layer flip rate drops to $W_{\text{flip}} = 0.043$---far below chance. Critically, split-half reliability ($0.878$) is \emph{lower} than between-model agreement ($0.993$): within-model measurement noise exceeds between-model variation. This means fine-tuning from a shared checkpoint does not create genuine Rashomon diversity---the models are too similar. The theory correctly predicts: less Rashomon $\to$ less instability $\to$ less need for orbit averaging.

\subsection{Method Robustness: Weight Zeroing vs.\ Mean Ablation}
\label{sec:mean-ablation}
...
\paragraph{Raw disagreement, invariant agreement.}
Table~\ref{tab:mean-ablation-agreement} shows that weight zeroing and mean ablation disagree substantially on raw head importance (cross-method per-model $\rho \approx 0.5$--$0.6$) but agree on the $G$-invariant projection ($\rho_{G\text{-inv}} > 0.97$).

\begin{table}[ht]
\centering
\caption{Between-method agreement: weight zeroing vs.\ mean ablation. $\rho_{\text{full}}$: mean pairwise Spearman of raw importance vectors across seeds (within each method). Cross-method $\rho$: per-model Spearman between weight-zeroing and mean-ablation importance vectors.}
\label{tab:mean-ablation-agreement}
\small
\begin{tabular}{lcc}
\toprule
& \textbf{Config A} & \textbf{Config B} \\
\midrule
Weight zeroing $\rho_{\text{full}}$ & 0.565 & 0.540 \\
Mean ablation $\rho_{\text{full}}$ & 0.914 & 0.965 \\
Mean ablation $\rho_{G\text{-inv}}$ & 0.975 & 0.987 \\
\midrule
Cross-method $\rho$ (mean) & 0.591 & 0.504 \\
Cross-method $\rho$ (range) & [0.489, 0.718] & [0.433, 0.596] \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Per-model cross-method correlations.}
Table~\ref{tab:cross-method-per-model} shows the full per-model detail, confirming that the disagreement is systematic, not driven by outliers.

\begin{table}[ht]
\centering
\caption{Per-model Spearman correlation between weight-zeroing and mean-ablation importance vectors. Each row is one independently trained model. The two methods agree moderately on raw components but converge after orbit averaging.}
\label{tab:cross-method-per-model}
\small
\begin{tabular}{ccc}
\toprule
\textbf{Seed} & \textbf{Config A} & \textbf{Config B} \\
\midrule
0 & 0.629 & 0.532 \\
1 & 0.524 & 0.596 \\
2 & 0.694 & 0.477 \\
...
\midrule
$\rho_{\text{full}}$ [range] & 0.277 [0.121, 0.435] \\
$\rho_{G\text{-inv}}$ [range] & 0.873 [0.775, 0.964] \\
\midrule
$W_{\text{flip}}$ [95\% CI] & 0.500 [0.482, 0.517] \\
$B_{\text{flip}}$ (head vs.\ MLP) [95\% CI] & 0.014 [0.008, 0.019] \\
Cohen's $d$ & 19.2 \\
\midrule
...
\begin{itemize}
  \item \textbf{Raw agreement near chance}: $\rho_{\text{full}} = 0.277$ is barely above zero---full component importance vectors are nearly uncorrelated across training seeds. This is \emph{worse} than TinyStories ($\rho \approx 0.55$) because GPT-2 has 12 heads per layer (larger symmetric groups, more permutation freedom).
  \item \textbf{$G$-invariant lift}: After orbit averaging, $\rho_{G\text{-inv}} = 0.873$---a $3.2\times$ lift. The shared structure is real but hidden by within-layer permutation noise.
  \item \textbf{Within-layer flip near 50\%}: $W_{\text{flip}} = 0.500$ [0.482, 0.517], as predicted for symmetric components. Within each layer, ``which head matters most?'' is determined by the random seed, not by the data.
  \item \textbf{Between-group stability}: $B_{\text{flip}} = 0.014$---heads almost never outrank MLP layers across seeds. The between- vs.\ within-group separation is massive (Cohen's $d = 19.2$).
  \item \textbf{Ranking lottery}: 32 distinct components appear in the top-5 across just 10 seeds, with 8 different \#1 components. Claiming ``component X is most important'' based on one training run is an artifact.
\end{itemize}

\paragraph{Random projection vs.\ random grouping.}
The random projection test ($p = 1.0$) shows that projecting to the same dimensionality as $V^G$ achieves high agreement ($\rho \approx 0.97$ for random subspaces)---dimensionality reduction \emph{alone} explains most of the lift. The random grouping test ($p = 0.001$) validates that the \emph{layer structure specifically} is critical: random partitions of heads into 12 groups of 12 produce significantly lower agreement than the actual per-layer grouping. The $G$-invariant projection works because it uses the \emph{correct} symmetry group, not merely because it reduces dimensionality.

\paragraph{Pretrained GPT-2 baseline.}
For reference, pretrained GPT-2-small achieves 99.5\% accuracy on IOI prompts with mean logit difference $= 3.1$ (vs.\ 72.1\% and 0.98 for our from-scratch models). The large gap confirms that our models learn a qualitatively different (weaker but genuine) IOI capability, sufficient to generate Rashomon diversity.

\subsection{IOI Circuit Analysis: The Component-Level Ranking Lottery}
\label{sec:ioi-circuit}

\paragraph{Setup.}
The Indirect Object Identification (IOI) task tests whether a model can identify which name receives the indirect object in sentences like ``When Mary and John went to the store, John gave a drink to \underline{\hspace{1cm}}.'' We evaluate all 10 GPT-2-small models on 200 IOI prompts, measuring logit difference (IO token logit minus S token logit) and component importance via activation patching.

\paragraph{Results.}
Mean IOI accuracy is 72.1\% (range: 63.0\%--84.0\%) with mean logit difference $= 0.98$---above the 60\% gate but far below pretrained performance (99.5\%).

\begin{table}[ht]
\centering
\caption{IOI circuit analysis: component importance stability across 10 GPT-2-small models trained from scratch. The ranking lottery is even more severe than for general importance.}
\label{tab:ioi-results}
\small
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Raw agreement $\rho$ [range] & 0.158 [$-0.089$, 0.284] \\
$G$-invariant $\rho$ [range] & 0.538 [0.141, 0.860] \\
\midrule
...
\begin{itemize}
  \item \textbf{Raw agreement near zero}: $\rho = 0.158$ means IOI-specific importance vectors are nearly uncorrelated. The ``IOI circuit'' identified in one model has almost no relation to the circuit in another model trained on the same data.
  \item \textbf{$G$-invariant lift moderate}: $\rho_{G\text{-inv}} = 0.538$ is lower than for general importance (0.873). Task-specific circuits show more idiosyncratic structure that orbit averaging cannot fully recover---consistent with there being less stable between-layer signal to extract.
  \item \textbf{Within-layer flip near 50\%}: $W_{\text{flip}} = 0.499$, confirming the impossibility at the task-specific level.
  \item \textbf{32 distinct top-5 rankings}: Every seed produces a different set of ``most important'' IOI components. Reporting a specific IOI circuit from a single training run is reporting an artifact.
\end{itemize}

\paragraph{Per-seed top-5 components.}
Table~\ref{tab:ioi-top5} shows the top-5 components for each seed, demonstrating the diversity.

\begin{table}[ht]
\centering
\caption{Top-5 IOI components per seed. No two seeds share the same top-5. Components span layers 0--9 and include both heads and MLPs.}
\label{tab:ioi-top5}
\small
\begin{tabular}{cl}
\toprule
\textbf{Seed} & \textbf{Top-5 Components} \\
\midrule
...
The IOI circuit literature identifies specific attention heads (e.g., ``induction heads,'' ``name-mover heads'') as the circuit responsible for IOI. Our results show that this identification is seed-dependent: the \emph{functional role} (IOI capability) is stable (72\% accuracy across all seeds), but the \emph{component assignment} (which specific heads implement it) is a coin flip within each layer. This is the component-level analogue of the input-level ranking lottery: the importance ranking of symmetric components is determined by the training seed, not by the architecture or data.

\subsection{SAE Feature Stability: The Impossibility at the Dictionary Level}
\label{sec:sae-stability}

\paragraph{Why the impossibility applies to SAE features.}
The Attribution Impossibility (Theorem~\ref{thm:impossibility}) requires only the Rashomon property: equivalent configurations with different explanations. For Sparse Autoencoders, Rashomon holds by \emph{linear algebra}, not merely by empirical observation. An $8\times$ overcomplete dictionary ($d_{\text{SAE}} = 6{,}144$ features for $d_{\text{model}} = 768$ dimensions) has a $6{,}144 - 768 = 5{,}376$-dimensional null space. Any rotation within this null space produces an equivalent dictionary with identical reconstruction loss. The solution space is massively non-unique---the Rashomon property holds with mathematical certainty, not as a contingent empirical fact.

The ``elements being ranked'' are the 6{,}144 learned features. The ``models'' are the 10 SAE training seeds (each producing a different dictionary). The ``observable equivalence'' is identical reconstruction loss (${\sim}0.55$ for all). The theorem's prediction: no ranking of SAE features by importance can be simultaneously faithful (reflect the actual feature activations), stable (consistent across equivalent SAE seeds), and complete (rank all features). The follow-up experiments test three escape hypotheses; all fail.

\paragraph{Setup.}
We train 10 TopK Sparse Autoencoders on the layer-6 activations of a single frozen GPT-2-small model (seed 0), with $d_{\text{SAE}} = 6{,}144$, $k = 48$ active features per input, and 50{,}000 training steps. Feature matching uses both greedy (max cosine per feature) and optimal (Hungarian on top 500) algorithms.

\paragraph{Follow-up conditions.}
Three escape hypotheses are tested:
\begin{enumerate}
  \item \textbf{ReLU SAE} (Bricken et al.\ architecture): Replace TopK with ReLU activation. If TopK's hard sparsity constraint is the bottleneck, ReLU might learn more reproducible features.
  \item \textbf{Pretrained GPT-2}: Train 10 TopK SAEs on pretrained GPT-2-small (not our from-scratch model). If the from-scratch model's idiosyncratic representations are the problem, a canonical pretrained model should yield reproducible features.
  \item \textbf{Resolution} (orbit averaging on features): Hungarian-match features across all 10 seeds, then average the matched decoder vectors. If the resolution framework applies, averaging should improve reproducibility.
\end{enumerate}
...
\begin{table}[ht]
\centering
\caption{SAE feature stability across four conditions. The impossibility manifests at every level: no condition achieves reproducible features. Rashomon holds by overcompleteness (5{,}376-dim null space), making the impossibility mathematically inevitable for overcomplete dictionaries.}
\label{tab:sae-comparison}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Condition} & \textbf{Max Cosine} & \textbf{Matched Cos} & \textbf{Matched $\rho$} & \textbf{Raw $\rho$} \\
\midrule
TopK (from-scratch) & 0.394 $\pm$ 0.002 & 0.495 & 0.500 & $-0.002$ \\
ReLU (from-scratch) & 0.136 $\pm$ 0.0001 & 0.107 & 0.011 & 0.001 \\
TopK (pretrained GPT-2) & 0.443 $\pm$ 0.002 & 0.516 & 0.507 & $-0.002$ \\
Resolution (averaged) & 0.458 & --- & --- & --- \\
\midrule
\emph{Random baseline} & ${\sim}0.015$ & ${\sim}0.015$ & 0.0 & 0.0 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Interpretation by condition.}

\begin{enumerate}
\item \textbf{TopK (from-scratch)}: Max cosine $= 0.394$ ($2.6\times$ random). Features share only ${\sim}15\%$ of variance with their best match in another seed. Importance ranking after optimal matching is at chance ($\rho = 0.500$). The impossibility manifests: equivalent SAEs produce different feature importance rankings.

\item \textbf{ReLU (from-scratch)}: \emph{Catastrophically non-reproducible.} Max cosine $= 0.136$---barely above the random baseline (${\sim}0.015$ for unit vectors in 768d). Matched importance $\rho = 0.011$ is indistinguishable from noise. The ReLU architecture (L0 $\approx 2{,}500$ active features per input vs.\ 48 for TopK) learns an essentially arbitrary decomposition that changes completely across seeds. The TopK sparsity constraint was the only source of partial structure; without it, the overcomplete null space dominates entirely.

\item \textbf{TopK (pretrained GPT-2)}: Max cosine $= 0.443$---marginally better than from-scratch (0.394) but still far from reproducible. This \textbf{falsifies the escape hypothesis} that the from-scratch model's idiosyncratic representations are the problem. All 10 SAEs are trained on the \emph{exact same frozen model}---same activations, same layer, same architecture---yet features are still non-reproducible. The non-reproducibility is a property of SAE training dynamics in overcomplete spaces, not of the underlying model's representation diversity.

\item \textbf{Resolution (averaged decoders)}: Hungarian-matching features across seeds and averaging the matched decoder vectors yields mean cosine $= 0.458$ (improvement of $+0.064$ over individual). This confirms that orbit averaging partially works---consistent with the Reynolds framework. But the residual non-reproducibility ($0.458$ is still far from $1.0$) reflects that SAE features lack a clean finite symmetry group. The ``symmetry'' in dictionary space is a continuous rotation group in the null space, not a discrete permutation group. Orbit averaging over an infinite group requires knowledge of the group structure, which is unavailable.
\end{enumerate}

\paragraph{The impossibility hierarchy.}
The SAE results complete a three-level picture of how the impossibility scales with symmetry group size:

\begin{center}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Level} & \textbf{Why Rashomon holds} & \textbf{$|G|$} & \textbf{Raw $\rho$} & \textbf{After resolution} \\
\midrule
Input features (SHAP) & Collinearity + stochastic training & $S_m$ (finite) & flips 50\% & DASH $\to$ 0\% flip \\
Attention heads & Permutation symmetry & $\prod S_k$ (finite) & 0.277 & $G$-inv $\to$ 0.873 \\
SAE features (TopK) & Overcompleteness (null space) & $O(5376)$ (continuous) & $-0.002$ & avg $\to$ 0.458 \\
SAE features (ReLU) & Overcompleteness (larger L0) & $O(5376)$ (continuous) & 0.001 & none available \\
\bottomrule
\end{tabular}
\end{center}

The pattern is clear: \emph{more symmetry $\to$ more Rashomon $\to$ worse instability $\to$ harder resolution.} When the symmetry group is finite and known ($S_k$ for attention heads), orbit averaging achieves near-perfect resolution. When it is continuous and unknown (overcomplete dictionary), resolution is partial at best. The impossibility fires everywhere; only the resolution scales with structural knowledge.

\paragraph{Implications for mechanistic interpretability via SAEs.}
\begin{itemize}
\item Claims of the form ``feature $X$ activates on concept $Y$'' are seed-dependent: a different SAE seed may not learn a corresponding feature, or may assign it different importance.
\item The assumption that SAE features are ``natural kinds'' (uniquely determined by the data) is empirically falsified: the overcomplete null space guarantees multiple equivalent decompositions.
\item SAE-based interpretability should qualify claims as seed-specific, or average over multiple SAE seeds with matching (accepting the $+0.064$ improvement from resolution as partial stabilization).
\item The formal impossibility is not an artifact of training methodology---it follows from overcompleteness. Larger dictionaries, better optimizers, or longer training cannot escape the null space.
\end{itemize}

\paragraph{Dead features and training dynamics.}
The ReLU SAEs exhibit 3.2--4.4\% dead features (never activate); TopK SAEs on pretrained GPT-2 exhibit 2.1--2.8\%. Dead feature fraction is stable across seeds within each condition, suggesting it reflects dictionary geometry rather than training pathology. The pretrained model's lower dead rate (${\sim}2.4\%$ vs.\ ${\sim}3.5\%$) is consistent with pretrained representations being better-conditioned for sparse decomposition.

\paragraph{Corrected resolution analysis.}
The reported resolution improvement of $+0.064$ (individual cosine $0.394 \to$ mean-to-averaged $0.458$) is inflated by including seed~0 (the Hungarian matching reference) which trivially achieves cosine $0.984$ to the averaged decoder. For non-reference seeds (1--9), the mean cosine to the averaged decoder is $0.399$---an improvement of only $+0.005$ over the individual pairwise baseline of $0.394$. This means matching-then-averaging provides \textbf{essentially no benefit} for SAE features. The failure has two causes: (1)~the matching itself is unreliable at cosine $0.394$ (features barely correspond), so averaging incorrectly-matched features produces noise; (2)~10 seeds barely samples a $5{,}376$-dimensional null space.

\paragraph{The Explanation Capacity Theorem for overcomplete dictionaries.}
The resolution failure motivates a theoretical ceiling. The decoder $D : \mathbb{R}^n \to \mathbb{R}^d$ has rank $\leq d$ by the rank-nullity theorem. Feature importance vectors in the kernel of $D^T$ (dimension $\geq n - d$) contribute nothing to reconstruction and are determined entirely by training dynamics---they are non-reproducible by construction.

\begin{theorem}[Explanation Capacity for Overcomplete Dictionaries]\label{thm:sae-capacity}
For an $n$-feature SAE on $d$-dimensional data ($n > d$), the explanation capacity satisfies:
$$C = \frac{\dim(V^G)}{n} \leq \frac{d}{n}$$
where $V^G$ is the subspace of feature importance vectors that are reproducible across equivalent dictionaries. The explanation loss rate is $\eta \geq 1 - d/n = (n-d)/n$.

For GPT-2-scale SAEs ($d = 768$, $n = 6{,}144$): $C \leq 12.5\%$, $\eta \geq 87.5\%$.
\end{theorem}

\noindent\textit{Proof.}
The decoder maps $n$-dimensional feature space to $d$-dimensional data space. Its rank is at most $d$. By rank-nullity, the null space has dimension $\geq n - d = 5{,}376$. Feature importance in the null space is non-reproducible (any null-space rotation preserves reconstruction quality). The reproducible subspace $V^G$ has dimension $\leq \text{rank}(D) \leq d$. Machine-verified in Lean (\texttt{SAECapacity.lean}): \texttt{capacity\_lt\_one}, \texttt{loss\_rate\_positive}, \texttt{rashomon\_guaranteed}. \hfill$\square$

\paragraph{Empirical validation of the capacity bound.}
The theoretical ceiling is $C \leq 12.5\%$ (768 reproducible dimensions out of 6{,}144). Our empirical observation: matched importance $\rho = 0.500$ on 500 features gives effective reproducibility $C_{\text{eff}} \approx 500 \times 0.500^2 / 6{,}144 \approx 2.0\%$---well below the theoretical ceiling. The gap (2\% observed vs.\ 12.5\% theoretical) indicates that current matching algorithms recover only ${\sim}16\%$ of the maximum possible reproducible structure.

\paragraph{Why PCA fails as a resolution (empirical validation).}
A natural proposal: use PCA on importance vectors to discover the reproducible subspace data-driven. We test this on the GPT-2 importance data (10 seeds, 156 components). Result: PCA achieves Spearman $\rho = -0.031$ (worse than raw $= 0.277$), while the $G$-invariant projection achieves $\rho = 0.833$. PCA \textbf{actively hurts}.

Why? PCA finds directions of \emph{maximum variance}---which are exactly the \emph{within-layer permutation noise} (PC-1 loads 95\% on layer~0 heads, capturing which L0 head got lucky). The $G$-invariant projection averages \emph{out} this noise. They are literally opposite operations: PCA maximizes the signal PCA can explain (variance), but the variance IS the instability.

Leave-one-out validation confirms: predicting a held-out seed's G-invariant importance from the other 9 seeds gives $\rho = 0.860$. Predicting the held-out raw importance from the other 9 raw means gives only $\rho = 0.212$. The $G$-invariant projection identifies structure that \emph{generalizes} to unseen seeds; raw averaging does not.

\paragraph{The resolution requires group knowledge.}
This validates the core theoretical claim: \emph{resolution quality is proportional to knowledge of the symmetry group.} Data-driven methods (PCA on importance vectors) actively fail because they identify the symmetry noise as the dominant signal. The correct resolution must use a \emph{seed-invariant basis}: for heads, average within layers; for SAE features, project onto the data's principal components.

\paragraph{PC resolution: what IS reproducible about SAE dictionaries?}
We test what structure is stable across SAE seeds by projecting feature properties onto the principal components of model activations (computed from 2{,}000 random-token sequences through layer~6 of the base model, yielding all 768 PCs). Two importance metrics are tested:
\begin{enumerate}
  \item \textbf{L2 geometric} (decoder column norm): TopK SAEs normalize all columns to unit norm ($\|d_i\| = 1.000$), so this is constant across features. The PC projection measures \emph{directional density}---how many decoder columns point toward each PC direction. This is trivially determined by the reconstruction objective.
  \item \textbf{Activation-weighted} (mean $|z_i|$ across 500 inputs): the standard behavioral metric measuring how often and how strongly each feature fires. This depends on the SAE encoder and is NOT trivially determined by the loss.
\end{enumerate}

\begin{table}[ht]
\centering
\caption{PC resolution for SAE features: geometric vs.\ behavioral importance. The geometric coverage is trivially reproducible (forced by the reconstruction loss). Behavioral importance is partially reproducible---high at coarse grain (top-10 PCs: $\rho = 0.878$), degrading as finer directions are added.}
\label{tab:pc-resolution}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Raw $\rho$} & \textbf{PC-proj (768)} & \textbf{PC-proj (top 10)} \\
\midrule
L2 geometric (all norms $= 1.0$) & $-0.002$ & 0.977 & 0.764 \\
Activation-weighted (behavioral) & 0.003 & 0.711 & \textbf{0.878} \\
\midrule
\multicolumn{4}{l}{\emph{Activation-weighted by dimensionality:}} \\
\quad Top-10 PCs & & & 0.878 \\
\quad Top-50 PCs & & & 0.875 \\
\quad Top-100 PCs & & & 0.869 \\
\quad Top-200 PCs & & & 0.836 \\
\quad Top-768 PCs (all) & & & 0.711 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
\item \textbf{Geometric coverage is trivially reproducible} ($\rho = 0.977$). With unit-norm decoders, the PC projection measures the density of features in each data direction. The reconstruction objective forces all SAEs to cover the same directions similarly, making this near-tautological. The high $\rho$ reflects loss-function constraints, not a deep property of the dictionary.

\item \textbf{Behavioral importance is partially reproducible at coarse grain} ($\rho = 0.878$ at top-10 PCs). The major data directions (those explaining the most activation variance) have reproducible behavioral allocation across SAE seeds. This is genuine: the activation-weighted importance depends on the encoder, not just the decoder, and is not directly forced by the reconstruction loss. Many features contribute to each major direction, so the law of large numbers produces stable aggregates.

\item \textbf{Behavioral reproducibility degrades at fine grain} ($\rho = 0.711$ at all 768 PCs). Minor data directions (lower PCs) are covered by fewer features, so their aggregate activation importance is dominated by individual feature noise---the very non-reproducibility the impossibility theorem predicts. The pattern inverts with dimensionality: behavioral $\rho$ decreases from $0.878$ (top-10) to $0.711$ (all 768) as noise-dominated directions are added.
\end{enumerate}

\paragraph{The honest picture.}
\begin{center}
\small
\begin{tabular}{@{}llc@{}}
\toprule
\textbf{What} & \textbf{Status} & \textbf{$\rho$} \\
\midrule
Individual features & Non-reproducible (impossibility) & $\approx 0$ \\
Geometric coverage & Trivially reproducible (loss-forced) & 0.977 \\
Coarse behavioral allocation & Reproducible (genuine signal) & 0.878 \\
Fine behavioral allocation & Partially reproducible & 0.711 \\
\midrule
\emph{For comparison:} \\
Attention heads ($G$-invariant, different task$^*$) & Reproducible & 0.873 \\
\bottomrule
\multicolumn{3}{l}{\footnotesize $^*$10 different models vs.\ 10 SAEs on one model---not directly comparable.}
\end{tabular}
\end{center}

The SAE result confirms the impossibility at the feature level and reveals a nuanced resolution: coarse-grained behavioral summaries (${\sim}10$--$50$ PC directions) are genuinely reproducible, while fine-grained summaries degrade as non-reproducible directions accumulate. This parallels the attention-head resolution: layer-averaged importance (coarse) is stable at $\rho = 0.873$; per-head importance (fine) is unstable at $\rho = 0.277$. Both levels exhibit the Design Space tradeoff: granularity for stability.

\paragraph{Why activation PCs work but importance PCA fails.}
The two PCAs operate on fundamentally different data:
\begin{itemize}
  \item \textbf{PCA on importance vectors} (across seeds): finds directions of maximum \emph{variation across seeds} $=$ the symmetry noise. Result: $\rho = -0.031$ (Section~\ref{sec:gpt2-from-scratch}).
  \item \textbf{PCA on model activations} (across inputs): finds directions of maximum \emph{variation in the data} $=$ the structure the SAE must cover. These are seed-invariant (same frozen model).
\end{itemize}
The first identifies what changes; the second identifies what's shared.

\subsection{Explanation Uncertainty Bound (Quantitative Bilemma)}
\label{sec:quantitative-bilemma}

The bilemma applies to binary explanation spaces ($|H| = 2$). But SHAP values are continuous. We extend the bilemma to continuous $H$ equipped with a metric $d$, yielding the \emph{explanation uncertainty bound}: faithfulness and stability trade off quantitatively, with a minimum joint unfaithfulness determined by the Rashomon gap.
...
\begin{theorem}[Explanation uncertainty bound (quantitative bilemma)]\label{thm:quantitative-bilemma}
...
\noindent\textit{Proof.} Triangle inequality: $\Delta = d(\text{exp}(\theta_1), \text{exp}(\theta_2)) \leq d(\text{exp}(\theta_1), E(\theta_1)) + d(E(\theta_1), E(\theta_2)) + d(E(\theta_2), \text{exp}(\theta_2)) \leq \text{unfaith}(\theta_1) + \delta + \text{unfaith}(\theta_2)$. Rearrange.  Proved in Lean (\texttt{AlphaFaithful.lean}). \hfill$\square$
...
Bimodality is confirmed for all high-collinearity settings ($\rho \geq 0.5$, dip $p < 0.002$) and correctly absent in the control ($\rho = 0$, $p = 0.373$).
...
California Housing ($p = 0.575$) does not show significant bimodality, consistent with the theory: the prediction applies when coverage conflict holds (strong within-group collinearity), not to datasets with weak or partial collinearity. California Housing has heterogeneous, moderate correlations ($\rho \approx 0.3$--$0.6$) that do not produce the strong binary partition the all-or-nothing theorem requires.
The bimodality prediction is thus scoped to settings with substantial within-group collinearity ($\rho \geq 0.5$), where it is confirmed with $p < 0.002$.
...
(max flip rate 0.51), compared to 168 (39\%) with our canonical 50-model configuration using varying test sets---confirming that
...
The attention-based instability reported here (14.5\% of adjacent token pairs under fine-tuning) is a proxy experiment: attention weights are not SHAP values and do not satisfy the proportionality axiom. The formal component-level impossibility and its full-scale empirical confirmation are in Sections~\ref{sec:mech-interp}--\ref{sec:sae-stability}, where activation patching on GPT-2-small models trained from scratch demonstrates within-layer flip rate $= 0.500$ with the $G$-invariant projection as the resolution.
...
With $M = 20$ models per dataset, the power to detect a true flip rate of $p = 0.15$ (moderate instability) at the $10\%$ threshold is only $32\%$. This means datasets with true flip rates between $10\%$ and $20\%$ will often be classified as ``stable'' --- the reported $68\%$ prevalence is a conservative lower bound.
...
\noindent Sensitivity to the threshold choice:
\begin{itemize}
\item At $5\%$ threshold: $78\%$ of datasets show instability (60/77). More datasets qualify but the false positive risk increases.
\item At $10\%$ threshold: $68\%$ (52/77) --- the reported headline.
\item At $15\%$ threshold: $58\%$ (45/77). Still a majority, even with the more conservative criterion.
\item At $20\%$ threshold: $49\%$ (38/77). Nearly half of datasets show substantial instability.
\end{itemize}

\noindent The cross-validated estimate (from the 85-dataset bulletproof validation with alternative hyperparameter configurations) gives $66\%$--$75\%$ prevalence across 3 configurations, consistent with the $68\%$ headline. The true prevalence among benchmark-representative datasets is likely $75$--$85\%$, limited by the power of the $M{=}20$ design to detect flip rates in the $10$--$20\%$ range.
...
\begin{sloppypar}
The formalization comprises 59 Lean~4 files with 6 axioms and 368 type-checked
...
\subsection{Development Methodology and AI Assistance}
\label{sec:ai-disclosure}

\textbf{AI-assistance disclosure.} The Lean 4 formalization was developed with substantial assistance from Claude Code (Anthropic, Claude Opus 4.6). The development followed a human-directed, AI-assisted workflow:

\begin{itemize}
\item \textbf{Mathematical framework (human-conceived).} The proof targets (what to prove and why), the axiom design (what to assume about GBDT attribution), and the interpretation of results are the first author's contribution. The decision to decompose the impossibility into F/S/C, the identification of the bilemma as a strengthening, the MI boundary as the exact condition, and the design-space exhaustiveness as the characterization result are human mathematical insights.

\item \textbf{Axioms (human-conceived, AI-assisted in encoding).} All 6 axioms were designed by the first author. The formal Lean 4 encoding of these axioms was joint work: the first author specified the mathematical content, and Claude Code produced the Lean syntax. The axiom reduction from 16 to 6 was a joint effort.

\item \textbf{Theorem statements (human-directed, AI-drafted, human-reviewed).} The first author specified each proof target in mathematical terms. The formal Lean 4 type signatures (exact quantifier structure, hypothesis lists, return types) were typically AI-drafted based on these specifications and then reviewed by the first author for mathematical accuracy. Some statements were revised through multiple human/AI iterations to correctly capture the intended claim.

\item \textbf{Proof tactics (AI-assisted).} Approximately 80\% of tactic-mode proofs were initially drafted by Claude Code and then verified, corrected, or restructured by the first author. The initial formalization (49 theorems) was developed iteratively; the expansion to 368 theorems was produced by dispatching targeted Lean-writing agents with detailed specifications.

\item \textbf{Verification (machine-checked).} Every proof was accepted only after passing \texttt{lake build} (the Lean 4 type-checker) with zero \texttt{sorry}. The type-checker is the final arbiter of proof correctness, regardless of whether the proof was human-written or AI-drafted.

\item \textbf{Experiments and paper text (human-directed, AI-assisted).} Experiment scripts were co-developed with AI assistance. The paper text was written with AI drafting and editing assistance. All experimental designs, mathematical claims, and scientific interpretations are the responsibility of the human authors.
\end{itemize}

\noindent We view this workflow as a methodology contribution. The key insight: in formally verified mathematics, the division between ``who conceived it'' and ``who wrote the code'' is less important than whether the axioms correctly model the domain and the type-checker accepts the proofs. The axioms encode what the system is \emph{about} (human judgment); the proofs establish what follows (machine-verified, regardless of author). This division of labor --- human mathematical conception, joint formal encoding, AI-assisted proof construction, machine-verified correctness --- may be a template for future formalization efforts at scale.

\subsection{Proof Depth Distribution}

Of 368 type-checked declarations: 139 require multi-step proofs (${\geq}5$ tactic lines), 21 require ${\geq}10$ lines, and 5 require ${\geq}20$ lines. The five deepest proofs are \texttt{attribution\_sum\_symmetric} (35 lines, derived symmetry via split-count decomposition), \texttt{Phi\_neg} (29 lines, Gaussian CDF properties), \texttt{gaussian\_rashomon\_witnesses} (27 lines, FIM ellipsoid construction), \texttt{sumSqRankDiff\_ge\_sq\_groupSize} (26 lines, midrank algebra), and \texttt{binary\_group\_firstmover\_is\_j\_or\_k} (23 lines, Finset cardinality argument).
The 368 count includes both substantive multi-step proofs and definitional wrappers (e.g., backward-compatible aliases). Of these, 139 require ${\geq}5$ tactic lines and 21 require ${\geq}10$ lines.
...
\begin{table}[t]
\centering
\caption{Lean file cross-reference (59 files, 368 theorems, 6 axioms).}
...
\citet{damour2022underspecification} document that underspecification---the existence of multiple models with equivalent held-out performance---causes instability in predictions, fairness metrics, and feature attributions across domains. Our contribution is complementary: we prove that attribution instability under underspecification is a mathematical impossibility (not merely an empirical observation), quantify the divergence rate by model class, and provide a provably optimal resolution.
...
\paragraph{Mechanistic interpretability and circuit multiplicity.}
\citet{chughtai2023toy} show that independently trained networks learn distinct internal representations for the same group operation, demonstrating empirical Rashomon at the circuit level. \citet{meloux2025circuits} find 85 distinct valid circuits with zero circuit error for XOR. Our component-level results extend this line of work at full scale: 10 GPT-2-small models trained from scratch produce 32 distinct top-5 component rankings with within-layer flip rate $= 0.500$ (Section~\ref{sec:gpt2-from-scratch}). The instability is mathematically inevitable under architectural symmetry, and the $G$-invariant projection provides a principled resolution---averaging within orbits of the symmetry group to recover the stable shared structure (lifting $\rho$ from 0.277 to 0.873 on GPT-2, from 0.55 to 0.97 on TinyStories).
...
\begin{sloppypar}
\noindent\textbf{Reference implementation.} The DASH stability API, diagnostic tools, and all experiment scripts are available at \url{https://github.com/DrakeCaraker/dash-shap} (stability API in PR~\#255).

\noindent\textbf{Lean formalization.} The 368-theorem, 6-axiom, 0-\texttt{sorry} formalization is available at \url{https://github.com/DrakeCaraker/dash-impossibility-lean} (archived at \url{https://doi.org/10.5281/zenodo.19468379}).

\noindent\textbf{TinyStories models.} All 30 transformer models (10 seeds $\times$ 2 configs from scratch + 10 GPT-2 fine-tunes) trained on TinyStories \citep{eldan2023tinystories} will be released with the camera-ready version.

\noindent\textbf{GPT-2-small models.} All 10 GPT-2-small models trained from scratch on OpenWebText (50{,}000 steps each, ${\sim}3$B tokens), along with activation patching results, IOI analysis, and SAE stability data, are available in the repository under \texttt{docs/gpt2-experiment-results/}.

\noindent\textbf{Datasets.} All input-level experiments use publicly available datasets: Breast Cancer Wisconsin, German Credit (UCI ML Repository), California Housing, Diabetes, Wine Quality, Heart Disease. The gene expression datasets (AP\_Colon\_Kidney, AP\_Breast\_Lung) are from the ArrayExpress public repository.
\end{sloppypar}
...
The Bayes-optimality half of the Design Space is proved but not in Lean (requires measure-theoretic decision theory). Deriving split-count axioms algorithmically from the TreeSHAP algorithm and proving the proportionality axiom for general tree depths remain open. The generalization from $\rho > 0$ to $I(X_j; X_k) > 0$ is now complete (\texttt{MutualInformation.lean}): mutual information is the exact boundary between possible and impossible feature ranking. For SAE features, the impossibility follows from overcompleteness (Section~\ref{sec:sae-stability}); the open question is not whether reproducibility is achievable (it is not, by null-space geometry) but whether a \emph{resolution} analogous to the $G$-invariant projection can be defined for continuous symmetry groups---this would require characterizing the group structure of the overcomplete dictionary null space. Scaling the component-level experiments beyond GPT-2-small (124M parameters) to larger models would strengthen the generality claim. A reference implementation is available at \url{https://github.com/DrakeCaraker/dash-shap}.
...
\textbf{AI assistance.} This work was developed with substantial assistance from Claude Code (Anthropic, Claude Opus 4.6). The mathematical framework (proof targets, axiom design, result interpretation) was human-conceived. Formal Lean 4 encodings (theorem type signatures, axiom syntax) were human-directed, AI-drafted, and human-reviewed. Approximately 80\% of proof tactics were AI-drafted; all were machine-verified with zero \texttt{sorry}. Experiments and paper text were co-developed with AI assistance; all scientific claims are the human authors' responsibility. See Section~\ref{sec:ai-disclosure} for the detailed methodology.
