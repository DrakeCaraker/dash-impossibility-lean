...
\begin{abstract}
No feature ranking can be simultaneously faithful, stable, and complete when features are collinear---an impossibility we prove, quantify for four model classes, resolve via \DASH{} ensemble averaging, and machine-verify with 188 Lean~4 theorems.
...
\textbf{Verification.} The framework is mechanically verified in Lean~4: 188 theorems from 18 axioms with 0 \texttt{sorry} across 36 files. The formalization caught two logical inconsistencies that survived informal review.
...
\begin{itemize}
\item \textbf{Art.~13 (Transparency):} Attribution instability under collinearity constitutes a ``known limitation'' that providers must disclose.
\item \textbf{Art.~14 (Human oversight):} Humans overseeing AI decisions must understand that SHAP rankings for correlated features may be arbitrary.
\item \textbf{Art.~17 (Quality management):} Providers should implement systematic instability testing (F5/F1 diagnostics) as part of quality management.
\item \textbf{Annex III:} The impossibility applies to all AI systems using feature attribution for explainability, including high-risk systems in credit, employment, and law enforcement.
\end{itemize}

\paragraph{Recommended actions.} Providers: run F5 screening on all models with correlated features; disclose instability for flagged pairs. Deployers: require DASH consensus or equivalent ensemble method for regulatory reporting. Authorities: update technical standards to require multi-model attribution testing.
