...
counts: ## Show current theorem/axiom/sorry/file counts
	@echo "Theorems+lemmas: $$(grep -c '^theorem\|^lemma' DASHImpossibility/*.lean | awk -F: '{s+=$$2} END {print s}')"
	@echo "Axioms:          $$(grep -c '^axiom' DASHImpossibility/*.lean | awk -F: '{s+=$$2} END {print s}')"
