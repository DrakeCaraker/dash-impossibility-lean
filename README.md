...
![Theorems](https://img.shields.io/badge/theorems-368-blue)
![Axioms](https://img.shields.io/badge/axioms-2-blue)
![Sorry](https://img.shields.io/badge/sorry-0-brightgreen)
![Lean 4](https://img.shields.io/badge/Lean-4-purple)
![Files](https://img.shields.io/badge/Lean_files-59-informational)

<!-- Verify badges with:
  grep -c '^theorem\|^lemma' DASHImpossibility/*.lean | awk -F: '{s+=$2}END{print s}'  # 368
...
For binary attribution questions (SHAP sign, feature selection, circuit membership), faithful + stable alone is impossible (the bilemma). The design space has exactly two families; DASH (orbit averaging) is Pareto-optimal. A 7-line nonparametric diagnostic outperforms the Gaussian formula by 2×. Everything is machine-verified in Lean 4: 368 theorems+lemmas across 59 files with 2 axiom declarations and 0 sorry.

> **Note on axiom counting.** Prose below referring to "the 6 axioms" describes the six behavioral assumptions of the axiom *system*; these are now carried as fields of 2 bundled `axiom` declarations (the keyword count changed, the assumed content did not). Canonical counts at HEAD: 59 files, 368 theorems+lemmas, 2 `axiom` declarations, 0 sorry.
...
├── DASHImpossibility/                    # 59 Lean 4 files, 368 theorems+lemmas, 2 axiom declarations, 0 sorry
│   │
│   │  ── Level 0: Pure Logic ──
...
│   └── Basic.lean                        # Import hub (all 59 files)
│
├── paper/
...
**368 theorems+lemmas. 2 axiom declarations. 0 sorry. 59 files. 15 abstraction levels. 139 multi-step proofs (>=5 tactic lines).**
...
4. **Axiom Consistency Model** — Python numerical verification of all 6 axioms
